...
// Creator:    pvesvslx                                                        
// Time:       Friday Aug 1, 2014 [11:06:10 am]                                
//                                                                             
...
//                                                                             
// Sources:    /tmp/pvesvslx/nebulon_run/30229/fxr_pmon.rdlh                   
//             /tmp/pvesvslx/nebulon_run/30229/lib_udp.rdl                     
//             /tmp/pvesvslx/nebulon_run/30229/fxr_pmon.rdl                    
...
// PMON_FREEZE_TIME desc:  This is the synchronized time at which the performance monitor
// counter array was frozen.
typedef volatile union {
    struct {
        uint64_t  FREEZE_TIME          :  64;    //  The synchronized time at
                                                 // which the counters were frozen
...
// PMON_ENABLE_TIME desc:  This is the synchronized time at which the performance monitor
// counter array was released from the freeze state.
typedef volatile union {
    struct {
        uint64_t  ENABLE_TIME          :  64;    //  The synchronized time at
                                                 // which the counters were
                                                 // enabled
...
// PMON_NETWORK_TIME desc:  This is the network synchronized time. When this register is read, a
// snapshot of the local TSC on the HFI is captured.
typedef volatile union {
    struct {
        uint64_t  NET_TIME             :  64;    //  The synchronized network time
...
// PMON_LOCAL_TSC desc:  This is the HFI based TSC time at the time the network time was read.
typedef volatile union {
    struct {
        uint64_t  HFI_TSC              :  64;    //  The HFI TSC that was captured
                                                 // when the network time was
                                                 // read.
...
typedef struct {
    ...
    PMON_NETWORK_TIME_t        PMON_NETWORK_TIME; // offset 16'h1000, width 64
    PMON_LOCAL_TSC_t           PMON_LOCAL_TSC;   // offset 16'h1008, width 64
