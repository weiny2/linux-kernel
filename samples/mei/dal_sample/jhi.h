...
#define JHI_VERSION "1.32.20190403"
...
/*-----------------------------------------------------------------------------
 *   JHI 7.1 return codes - not used in 8.0 and above
 *   (backward compatibility only), JHI 8.0 return codes are listed below
------------------------------------------------------------------------------*/
...
#define JHI_FILE_UUID_MISMATCH      0x107        // UUIDs don't match between applet file and function input
...
//
// Note that this size limitation does not mark the maximum buffer size an applet can receive,
...
*/
#define MK_APPLET_VER(maj, min) \
    ((uint32_t)((maj) & 0x000000FFUL) |  \
                (((min) << 8) & 0x0000FF00UL) &  \
                 (0x0000FFFFUL) )

/* Extract Applet Major Version from a VM integer representation (num) */
#define MAJOR_APPLET_VER(num)  ((uint8_t)  (num & 0x000000FFUL))

/* Extract Applet Minor Version from a VM integer representation (num) */
#define MINOR_APPLET_VER(num)  ((uint8_t) ((num & 0x0000FF00UL) >> 8))
...
typedef struct {
    uint32_t datalen;                    // byte length of the event data
    uint8_t* data;                    // the buffer that contains the event data
...
// this enum lists the platform types that are supported by JHI
...
typedef struct
{
    ...
    uint32_t          flags;        // the flags used when this session created
...
typedef struct
{
   ...
   //--------------------!!!!!!!!!!!--------------------//
   // Don't add members in between TxBuf[1] & RxBuf[1]   //
...
JHI_SendAndRecv2(
    ...
    IN int32_t            nCommandId,
    INOUT JVM_COMM_BUFFER* pComm,
    OUT int32_t* responseCode);
