...
dst_build=`ls /nfs/ph/proj/stlbuilds/System_Test | grep OPENIB_INSTALL_10_0_0_0_* | awk 'BEGIN { FS = "_"} ; {print $7}' | sort -n | tail -n 1`
echo "$dst_build"
...
cp $ifs_build/X86_64_RHEL6/IntelOPA-IFS.RHEL6-x86_64.10.0.0.0.$build_num.tgz $ifs_priv
if [ $? -ne 0 ]; then
	echo "Could not copy RHEL 6 tarball"
	exit 1
fi

cp $ifs_build/X86_64_RHEL7/IntelOPA-IFS.RHEL7-x86_64.10.0.0.0.$build_num.tgz $ifs_priv
if [ $? -ne 0 ]; then
...
tar xzf $ifs_priv/IntelOPA-IFS.RHEL6-x86_64.10.0.0.0.$build_num.tgz
if [ $? -ne 0 ]; then
...
tar xzf $ifs_priv/IntelOPA-IFS.RHEL7-x86_64.10.0.0.0.$build_num.tgz 
if [ $? -ne 0 ]; then
...
echo "---------------------------"
echo "OFED"
rsync -avh -n --ignore-existing --exclude 'hfi-*' IntelOPA-IFS.RHEL6-x86_64.10.0.0.0.$build_num/IntelOPA-OFED.RHEL6-x86_64.10.0.0.0.*/RPMS/redhat-ES6/ $yum6
echo "FM"
rsync -avh -n --ignore-existing --exclude 'hfi-*' IntelOPA-IFS.RHEL6-x86_64.10.0.0.0.$build_num/IntelOPA-FM.RHEL6-x86_64.10.0.0.0.*/RPMS/x86_64/ $yum6
echo "FF"
rsync -avh -n --ignore-existing --exclude 'hfi-*' IntelOPA-IFS.RHEL6-x86_64.10.0.0.0.$build_num/IntelOPA-Tools-FF.RHEL6-x86_64.10.0.0.0.*/RPMS/x86_64/ $yum6
...
echo "FF"
rsync -avh -n --ignore-existing --exclude 'hfi-*' IntelOPA-IFS.RHEL7-x86_64.10.0.0.0.$build_num/IntelOPA-Tools-FF.RHEL7-x86_64.10.0.0.0.*/RPMS/x86_64/ $yum7

echo "Press ENTER to update the repos or Ctrl+C to bail"
read resp
echo "Applying updates"
rsync -avh --ignore-existing --exclude 'hfi-*' IntelOPA-IFS.RHEL6-x86_64.10.0.0.0.$build_num/IntelOPA-OFED.RHEL6-x86_64.10.0.0.0.*/RPMS/redhat-ES6/ $yum6
if [ $? -ne 0 ]; then
	echo "Could not apply RHEL 6 - OFED"
	exit 1
fi

rsync -avh --ignore-existing --exclude 'hfi-*' IntelOPA-IFS.RHEL6-x86_64.10.0.0.0.$build_num/IntelOPA-FM.RHEL6-x86_64.10.0.0.0.*/RPMS/x86_64/ $yum6
if [ $? -ne 0 ]; then
	echo "Could not apply RHEL 6 - FM"
	exit 1
fi

rsync -avh --ignore-existing --exclude 'hfi-*' IntelOPA-IFS.RHEL6-x86_64.10.0.0.0.$build_num/IntelOPA-Tools-FF.RHEL6-x86_64.10.0.0.0.*/RPMS/x86_64/ $yum6
if [ $? -ne 0 ]; then
...
rsync -avh --ignore-existing --exclude 'hfi-*' IntelOPA-IFS.RHEL7-x86_64.10.0.0.0.$build_num/IntelOPA-FM.RHEL7-x86_64.10.0.0.0.*/RPMS/x86_64/ $yum7
if [ $? -ne 0 ]; then
	echo "Could not apply RHEL 7 - FM"
	exit 1
fi

rsync -avh --ignore-existing --exclude 'hfi-*' IntelOPA-IFS.RHEL7-x86_64.10.0.0.0.$build_num/IntelOPA-Tools-FF.RHEL7-x86_64.10.0.0.0.*/RPMS/x86_64/ $yum7
if [ $? -ne 0 ]; then
