...
#these tests are expected to fail (intentionally)
fail_test_list = [ "MPI_Abort_c",
                   "MPI_Errhandler_fatal_c",
                   "MPI_Finalize_c"
               ]
#these tests are expected to pass
pass_test_list = [ "MPI_Address_c",
                   "MPI_Allgather_c",
                   "MPI_Allgatherv_c",
                   "MPI_Allreduce_c",
                   "MPI_Allreduce_loc_c",
...
skipped_test_list = [ "MPI_Comm_compare_null_c"
                      # result of comm_compare with a null communicator
                      # is undefined, so we can't test for this
                  ]
...
def main():
    ...
    if base_dir == "":
        if test_info.is_mpiverbs():
            base_dir = test_info.mpiverbs_path + "/tests/intel_mpitest/"
        else:
            base_dir = test_info.mpipsm_path + "/tests/intel_mpitest/"
