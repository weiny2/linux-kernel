...
def main():
    ...
    RegLib.test_log(0, "Test: IbWriteBwRC.py started")
	...
	for i in range(3):
		...
		if ext_lid:
		    cmd = "ib_write_bw -m 4096 -d %s -n 5 -u 21 -p %d -s %s -x 1" % (dev, test_port, size)
		else:
		    cmd = "ib_write_bw -m 4096 -d %s -n 5 -u 21 -p %d -s %s" % (dev, test_port, size)
	...
	for test_port in test_ports:
        ...
        if test_info.is_qib():
            ...
            dev = "hfi1_0"
	if ext_lid:
            cmd = "ib_write_bw -m 4096 -d %s -n 5 -u 21 -p %d -s %s -x 1 %s" % (dev, test_port, size,
                                                                server_name)
        else:
            cmd = "ib_write_bw -m 4096 -d %s -n 5 -u 21 -p %d -s %s %s" % (dev, test_port, size,
                                                                server_name)
