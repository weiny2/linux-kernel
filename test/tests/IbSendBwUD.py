...
def main():
    ...
    RegLib.test_log(0, "Test: IbSendBwUD.py started")
    ...
    for size in sizes:
        ...
        child_pid = os.fork()
        if child_pid == 0:
            cmd = "ib_send_bw -d hfi0 -c UD -n 5 -p %d -s %s" % (test_port, size)
        ...
        cmd = "ib_send_bw -d hfi0 -c UD -n 5  -p %d -s %s %s" % (test_port, size,
                                                                 server_name)
