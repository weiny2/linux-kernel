...
#else
static ssize_t
hostfs_aio_read(struct kiocb *kiocb, struct iov_iter *to)
{
	DPRINT1("hostfs: hostfs_aio_read %ld\n",
		kiocb->ki_filp->f_dentry->d_inode->i_ino);
	if (!hostfs_revalidate_inode(kiocb->ki_filp->f_dentry->d_inode))
...
#else
static ssize_t
hostfs_aio_write(struct kiocb *kiocb, struct iov_iter *from)
{
	DPRINT1("hostfs: hostfs_aio_write %ld\n",
		kiocb->ki_filp->f_dentry->d_inode->i_ino);
	if (!hostfs_revalidate_inode(kiocb->ki_filp->f_dentry->d_inode))
...
#else
	.read_iter = hostfs_aio_read,
	.write_iter = hostfs_aio_write,
#endif
