...
int simics_readlink_copy(char __user *buffer, int buflen, const char *link)
{
...
static int
hostfs_getattr(const struct path *path, struct kstat *stat,
	       u32 request_mask, u32 flags)
{
	DPRINT1("hostfs: getattr\n");
	if (!hostfs_revalidate_inode(path->dentry->d_inode))
		return 0;
...
#else
	return simics_readlink_copy(buf, size, data.filename);
#endif
}
