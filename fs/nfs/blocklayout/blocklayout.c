...
static void bl_write_cleanup(struct work_struct *work)
{
	...
	if (likely(!wdata->header->pnfs_error)) {
		struct pnfs_block_layout *bl = BLK_LSEG2EXT(wdata->header->lseg);
		u64 start = wdata->args.offset & (loff_t)PAGE_CACHE_MASK;
		u64 end = (wdata->args.offset + wdata->args.count +
