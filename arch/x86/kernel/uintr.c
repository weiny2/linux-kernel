...
{
	return(t->thread.ui_recv->upid->puir != 0);
}
...
inline void uintr_switch(struct task_struct *prev, struct task_struct *next)
{
	...
	if (is_uvec_active(prev)) {
		...
		prev->thread.ui_recv->upid->sc.sn = 1;
	...
	if (is_uipi_active(next)) {
		wrmsrl_safe(MSR_IA32_UINT_TT, ((u64)next->thread.ui_send->uitt)
						| 1);
	...
	if (is_uvec_active(next)) {
		next->thread.ui_recv->upid->sc.ndst =
				cpu_to_ndst(smp_processor_id());

		wrmsrl_safe(MSR_IA32_UINT_PD, (u64)next->thread.ui_recv->upid);
		...
		next->thread.ui_recv->upid->sc.sn = 0;
...
static void *uintr_alloc_uitt(struct task_struct *task, unsigned int nr_entries)
{
	return kzalloc(sizeof(struct uintr_uitte) * nr_entries, GFP_KERNEL);
}

static void uintr_free_uitt(void *uitt)
{
	kfree(uitt);
}
...
{
	task->thread.ui_send->uitt[entry].target_upid_addr = (u64)&dummy_upid;
}
...
void uintr_unregister_sender(struct task_struct *t, unsigned int entry)
{
	...
	memset(&t->thread.ui_send->uitt[entry], 0, sizeof(struct uintr_uitte));
	clear_bit(entry, (unsigned long *)&t->thread.ui_send->uitt_mask);

	if (!t->thread.ui_send->uitt_mask) {
		uintr_free_uitt(t->thread.ui_send->uitt);
		t->thread.ui_send->uitt = NULL;
...
static int uintr_sender_init(struct task_struct *t)
{
	...
	t->thread.ui_send->uitt = uintr_alloc_uitt(current, UINTR_MAX_UITT_NR);
	if (!t->thread.ui_send->uitt) {
		kfree(t->thread.ui_send);
		t->thread.ui_send = NULL;
		return -ENOSPC;
	}
	...
	if (!msr64) {
		wrmsrl_safe(MSR_IA32_UINT_TT, ((u64)t->thread.ui_send->uitt) |
						1);
...
int uintr_register_sender(struct task_struct *recv_task, int uvec_no)
{
	...
	dump_uitt(ui_send->uitt, UINTR_MAX_UITT_NR);
	...
	uitte = &ui_send->uitt[entry];
	uitte->valid = 1;
	uitte->user_vec = uvec_no;
	uitte->target_upid_addr = (u64)recv_task->thread.ui_recv->upid;
...
/* Replace with self managed memory allocater */
static void uintr_free_upid(void *upid)
{
	kfree(upid);
}

static void *uintr_alloc_upid(void)
{
	return kzalloc(sizeof(struct uintr_upid_struct), GFP_KERNEL);
}
...
static int uintr_register_handler(u64 handler)
{
	...
	wrmsrl_safe(MSR_IA32_UINT_PD, (u64)upid);
...
{
	task->thread.ui_recv->upid->puir |= uvec;
}
...
void uintr_free_uvec(struct task_struct *t, int uvec)
{
	...
	uintr_free_upid((void *)t->thread.ui_recv->upid);
	t->thread.ui_recv->upid = NULL;
...
void uintr_free(struct task_struct *t)
{
	...
	// Free upid
	if (is_uvec_active(t)) {
		uintr_free_upid(t->thread.ui_recv->upid);
		t->thread.ui_recv->upid = NULL;
		kfree(t->thread.ui_send);
		t->thread.ui_send = NULL;
	}
...
void uintr_print_debug_info(void)
{
	...
	if (current->thread.ui_send)
		dump_uitt(current->thread.ui_send->uitt, UINTR_MAX_UITT_NR);

	if (current->thread.ui_recv) {
		sc = current->thread.ui_recv->upid->sc.raw;
		puir = current->thread.ui_recv->upid->puir;
	}
