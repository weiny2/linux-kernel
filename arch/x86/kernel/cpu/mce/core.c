...
void SVOSMceSetUserHandlerBankOwnership(int bank)
{
	svos_user_handler_owns_bank[bank] = 1;
}

void SVOSMceClearAllUserHandlerBankOwnership(void)
{
	int	bank;

	for (bank = 0; bank < MAX_NR_BANKS; bank++) {
		svos_user_handler_owns_bank[bank] = 0;
	}
}

EXPORT_SYMBOL(SVOSMceSetUserHandlerBankOwnership);
EXPORT_SYMBOL(SVOSMceClearAllUserHandlerBankOwnership);
#endif
