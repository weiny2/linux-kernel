...
MODULE_DESCRIPTION("Intel kcopy driver");
					...
					unsigned long n)
{
	...
	if (!pages) {
		pr_warn(
			"kcopy: kmalloc could not allocate %ld bytes\n",
			PAGE_SIZE);
		ret = -ENOMEM;
		goto bail;
	}
...
static int __init kcopy_init(void)
{
	...
	if (IS_ERR(kcopy_dev.class)) {
		...
		pr_err("kcopy: Could not create device class (err %d)\n", -ret);
		goto bail_chrdev;
	}
	...
	if (ret < 0) {
		pr_err("kcopy: Could not add cdev (err %d)\n", -ret);
		goto bail_class;
	}
	...
	for (i = 0; i < KCOPY_MAX_MINORS; i++) {
		...
		if (IS_ERR(kcopy_dev.devp[i])) {
			...
			pr_err("kcopy: Could not create devp %d (err %d)\n",
				i, -ret);
			goto bail_cdev_add;
		}
