...
static ssize_t intel_fwa_noidle_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%s\n", acpi_desc->intel_fwa_noidle ? "Y" : "N");
}

static ssize_t intel_fwa_noidle_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (rc)
		return rc;
	acpi_desc->intel_fwa_noidle = val;
	return size;
}
DEVICE_ATTR_RW(intel_fwa_noidle);
		...
		struct nvdimm_bus_descriptor *nd_desc)
{
	 ...
	 * but it need only be retrieved once / is assumed that it does
	 * not change.
	 */
	if (acpi_desc->fwa_cap == NVDIMM_FWA_CAP_INVALID) {
		...
		else if (acpi_desc->intel_fwa_noidle && (info.capability
...
static int intel_bus_fwa_activate(struct nvdimm_bus_descriptor *nd_desc)
{
	...
	struct {
		 ...
		 */
		.cmd = {
			.iodev_state = acpi_desc->intel_fwa_noidle
	...
	if (intel_bus_fwa_state(nd_desc) != NVDIMM_FWA_ARMED)
		return -ENXIO;
