...
static bool intel_fwa_noidle;
module_param(intel_fwa_noidle, bool, 0600);
MODULE_PARM_DESC(intel_fwa_noidle,
		"Skip platform injected idle over firmware activate");
		...
		struct nvdimm_bus_descriptor *nd_desc)
{
	...
	if (acpi_desc->fwa_cap == NVDIMM_FWA_CAP_INVALID) {
		...
		else if (intel_fwa_noidle && (info.capability
