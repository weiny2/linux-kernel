				...
				struct cxl_cper_event_rec *rec)
{
	if (rec->hdr.length <= sizeof(rec->hdr) ||
		...
		pr_err(FW_WARN "Invalid section length (%u)\n",
		       rec->hdr.length);
		return;
	}
...
int cxl_cper_register_callback(cxl_cper_callback notifier)
{
	guard(rwsem_write)(&cxl_cper_rw_sem);
	if (cper_callback)
		return -EINVAL;
	cper_callback = notifier;
	return 0;
}
EXPORT_SYMBOL_NS_GPL(cxl_cper_register_callback, CXL);

int cxl_cper_unregister_callback(cxl_cper_callback notifier)
{
	guard(rwsem_write)(&cxl_cper_rw_sem);
	if (notifier != cper_callback)
		return -EINVAL;
	cper_callback = NULL;
	return 0;
}
