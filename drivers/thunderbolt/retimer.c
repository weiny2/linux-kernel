...
static int tb_retimer_nvm_authenticate(struct tb_retimer *rt)
{
	u32 status;
	int ret;
	...
	/*
	 * Need to wait minimum of 5 seconds for the retimer to
	 * authenticate the new NVM. We wait sligthly more just to be on
	 * the safe side.
	 */
	msleep(5100);

	/*
	 * Once it comes back it may not have index set anymore so we
	 * need to issue ENUMERATE_RETIMERS first.
	 */
	ret = usb4_port_enumerate_retimers(rt->port);
	if (ret)
		return ret;

	/*
	 * Now we should be able to read the authentications status
	 * again from the metadata register.
	 */
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val) {
		...
		ret = tb_retimer_nvm_authenticate(rt);
		if (ret)
			goto exit_unlock;
	}
...
static int tb_retimer_add(struct tb_port *port, u8 index)
{
	...
	dev_info(&rt->dev, "new device found, vendor=%#x device=%#x\n",
		 rt->vendor, rt->device);
...
{
	dev_info(&rt->dev, "device disconnected\n");
	tb_nvm_free(rt->nvm);
