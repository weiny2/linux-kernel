...
static int tb_usb3_init(struct tb_tunnel *tunnel)
{
	...
	// TODO: This needs to work with discovery as well!

	/*
	 * Check that we have enough bandwidth available for the new
	 * USB3 tunnel.
	 */
	max_rate = tb_usb3_max_link_rate(tunnel->src_port, tunnel->dst_port);
	if (max_rate < 0)
		return max_rate;

	/* Only 90% can be allocated for USB3 isochronous transfers */
	max_rate = max_rate * 90 / 100;
	tb_tunnel_dbg(tunnel, "required bandwidth for USB3 tunnel %d Mb/s\n",
		      max_rate);

	if (max_rate > tunnel->max_up || max_rate > tunnel->max_down) {
		tb_tunnel_warn(tunnel, "not enough bandwidth for USB3 tunnel\n");
		return -ENOSR;
	}
...
static int tb_usb3_activate(struct tb_tunnel *tunnel, bool activate)
{
	...
	if (tb_port_is_usb3_up(tunnel->dst_port)) {
		res = tb_usb3_port_enable(tunnel->dst_port, activate);
		if (res)
			return res;

		res = usb4_usb3_port_wait_for_link(tunnel->src_port, true, 500);
		if (res)
			return res;
	}
				       ...
				       int max_down)
{
	...
	if (!tb_route(down->sw)) {
		tunnel->allocated_up = max_up;
		tunnel->allocated_down = max_down;
