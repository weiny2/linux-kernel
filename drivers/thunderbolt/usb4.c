...
static bool link_is_usb4(struct tb_switch *sw)
{
	struct tb_port *upstream;
	u32 val;

	upstream = tb_upstream_port(sw);
	if (!upstream->cap_usb4)
		return false;

	if (tb_port_read(upstream, &val, TB_CFG_PORT,
			 upstream->cap_usb4 + PORT_CS_18, 1))
		return false;
...
int usb4_switch_setup(struct tb_switch *sw, bool restore)
{
	...
	sw->link_usb4 = link_is_usb4(sw);
	tb_sw_dbg(sw, "link is %s\n", sw->link_usb4 ? "USB4" : "TBT3");
 ...
 * @port: USB4 port
 * @index: Retimer index
 * @status: Raw status code read from metadata (can be %NULL)
 *
 ...
 * Returns negative errno if there authentication command itself fails
 * or the retimer is not accessible anymore. Returns %0 if the last
 * authentication command was successful and %1 if the NVM image
 * authentication failed (in this case @status contains the result).
 */
