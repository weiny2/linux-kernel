...
int usb4_switch_setup(struct tb_switch *sw, bool restore)
{
	...
	if (tb_can_tunnel_usb3() && sw->link_usb4 &&
...
int usb4_switch_set_sleep(struct tb_switch *sw)
{
	...
	/*
	 * Enable wake coming from all USB4 downstream ports (from child
	 * routers). For device routers also do this for the upstream
	 * port. It should be set by default but do it anyway here.
	 */
	tb_switch_for_each_port(sw, port) {
		if (!tb_port_has_remote(port) ||
...
/**
 * usb4_port_unconfigure() - Set USB4 port un-configured
 * @port: USB4 router
 *
 ...
 *
 * Returns %0 if the authentication status was succesfully read. The
...
/**
 * usb4_usb3_port_wait_for_link() - Wait for USB3 link
 * @port: USB3 adapter port
 * @valid: Wait for link valid or not
 * @timeout_msec: Timeout to wait in ms
 *
 * Waits for USB3 link to become valid or invalid. Returns %0 in case of
 * success and negative errno in case of failure. In particular returns
 * %-ETIMEDOUT if the link state has not changed within @timeout_msec.
 */
int usb4_usb3_port_wait_for_link(struct tb_port *port, bool valid,
				 int timeout_msec)
{
	u32 value = valid ? ADP_USB3_CS_4_ULV : 0;

	if (!tb_port_is_usb3_down(port) && !tb_port_is_usb3_up(port))
		return -EINVAL;

	/* TGL A step does not have this register implemented always active */
	if (tb_switch_is_tiger_lake_astep(port->sw))
		return 0;

	return usb4_port_wait_for_bit(port, port->cap_adap + ADP_USB3_CS_4,
				     ADP_USB3_CS_4_ULV, value, timeout_msec);
}
...
/**
 * usb4_usb3_port_allocated_bandwith() - Bandwidth allocated for USB3
 * @port: USB3 adapter port
