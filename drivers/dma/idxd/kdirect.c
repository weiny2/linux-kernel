...
static const void __iomem *idxd_dma_get_submission_portal(struct dma_chan *chan,
							  u64 *size)
{
	struct idxd_wq *wq = to_idxd_wq(chan);
			     ...
			     unsigned long flags, int timeout)
{
	...
	d->done = &done;
	rc = idxd_submit_desc(wq, d, IDXD_OP_BLOCK);
	if (rc < 0)
		return rc;
...
void idxd_setup_dma_kdirect(struct idxd_device *idxd)
{
	...
	dma->kdops.device_get_submission_portal =
		idxd_dma_get_submission_portal;
