...
static uint64_t idxd_pci_config[] = {
...
static uint64_t idxd_pci_ext_cap[] = {
...
static uint64_t idxd_cap_ctrl_reg[] = {
...
static int kvmidxd_guest_init(struct mdev_device *mdev)
{
	...
	info = vzalloc(sizeof(struct kvmidxd_guest_info));
	if (!info)
		return -ENOMEM;
...
static void __idxd_vdcm_release(struct vdcm_idxd *vidxd)
{
	...
	struct idxd_vdev *vdev = &vidxd->vdev;
	...
	rc = vfio_unregister_notifier(mdev_dev(vidxd->vdev.mdev),
			VFIO_GROUP_NOTIFY, &vidxd->vdev.group_notifier);
	if (rc < 0)
	...
	iommu_unregister_device_fault_handler(&vidxd->idxd->pdev->dev);
...
static struct vdcm_idxd *vdcm_vidxd_create(struct idxd_device *idxd,
		struct mdev_device *mdev, struct vdcm_idxd_type *type)
{
	struct vdcm_idxd *vidxd;
	unsigned long flags;
	...
	vidxd = kzalloc(sizeof(struct vdcm_idxd), GFP_KERNEL);
	if (!vidxd)
		return NULL;
	...
	vidxd->wq = wq;
	vidxd->vdev.cdev.pdev = idxd->pdev;
	...
	mutex_init(&vidxd->vdev.cdev.igate);
...
static int idxd_vdcm_create(struct kobject *kobj, struct mdev_device *mdev)
{
	...
	if (IS_ERR_OR_NULL(vidxd)) {
		rc = vidxd == NULL ? -ENOMEM : PTR_ERR(vidxd);
...
static int idxd_vdcm_open(struct mdev_device *mdev)
{
	...
	rc = vfio_pci_init_dma_fault_region(&vidxd->vdev.cdev);
	if (rc < 0) {
		dev_err(dev, "dma fault region init failed!\n");
		goto undo_guest;
	} else
		dev_dbg(dev, "dma fault region init successfully.\n");

	rc = vfio_pci_register_irq(&vidxd->vdev.cdev, VFIO_IRQ_TYPE_NESTED,
			VFIO_IRQ_SUBTYPE_DMA_FAULT, VFIO_IRQ_INFO_EVENTFD);
	if (rc < 0) {
		dev_err(dev, "register dma fault ext irq failed.\n");
		goto undo_guest;
	} else
		dev_dbg(dev, "register dma fault ext irq successful.\n");
...
{
	struct vdcm_idxd *vidxd = mdev_get_drvdata(mdev);
	struct vfio_pci_common_dev *cdev = &vidxd->vdev.cdev;
	...
	case VFIO_PCI_ROM_REGION_INDEX:
		dev_err(dev, "unsupported index: %u\n", index);
		break;
	default:
		dev_dbg(dev, "vendor specific region: %u\n", index);
		index -= VFIO_PCI_NUM_REGIONS;
		return cdev->region[index].ops->rw(cdev, buf, count,
				ppos, mode);
	}
...
static int idxd_vdcm_mmap(struct mdev_device *mdev, struct vm_area_struct *vma)
{
	...
	pgoff = (base + idxd_get_wq_portal_full_offset(wq->id, phys_limited,
...
static int idxd_vdcm_get_irq_count(struct vdcm_idxd *vidxd, int type)
{
	...
	if (type == VFIO_PCI_MSI_IRQ_INDEX ||
		...
		return vidxd->num_wqs + 1;
			...
			&& type < VFIO_PCI_NUM_IRQS + cdev->num_ext_irqs)
		return 1;

	return 0;
}

static int vdcm_idxd_set_msix_trigger(struct vfio_pci_common_dev *cdev,
		unsigned int index, unsigned int start, unsigned int count,
		uint32_t flags, void *data)
{
	struct idxd_vdev *vdev = to_ivdev(cdev);
	struct vdcm_idxd *vidxd = to_vidxd(vdev);
	...
	if (count == 0 && (flags & VFIO_IRQ_SET_DATA_NONE)) {
		...
		for (i = 0; i < VIDXD_MAX_MSIX_ENTRIES; i++) {
			if (vdev->msix_trigger[i]) {
				dev_dbg(dev, "disable MSIX entry %d\n", i);
				eventfd_ctx_put(vdev->msix_trigger[i]);
				vdev->msix_trigger[i] = NULL;
	...
	for (i = 0; i < count; i++) {
		if (flags & VFIO_IRQ_SET_DATA_EVENTFD) {
			...
			if (IS_ERR(trigger)) {
				dev_err(dev, "eventfd_ctx_fdget failed %d\n",
						i);
				return PTR_ERR(trigger);
			}
			vdev->msix_trigger[i] = trigger;
			/*
...
static int idxd_vdcm_set_irqs(struct vdcm_idxd *vidxd, uint32_t flags,
		...
		void *data)
{
	int (*func)(struct vfio_pci_common_dev *cdev, unsigned int index,
			unsigned int start, unsigned int count,
			uint32_t flags, void *data) = NULL;
	struct mdev_device *mdev = vidxd->vdev.mdev;
	struct device *dev = mdev_dev(mdev);
	...
	/*
	 * count comes from user space via VFIO. It needs to be boundary
	 * checked in case it's something crazy.
	 */
	if (count > msixcnt)
		count = msixcnt;
	...
	case VFIO_PCI_MSIX_IRQ_INDEX:
		...
		break;
	default:
		if ((flags & VFIO_IRQ_SET_ACTION_TYPE_MASK) ==
				VFIO_IRQ_SET_ACTION_TRIGGER) {
	...
	return func(&vidxd->vdev.cdev, index, start, count, flags, data);
}
	...
	if (cmd == VFIO_DEVICE_GET_INFO) {
		...
		default: {
			...
			if (cdev->region[i].ops->add_capability) {
				rc = cdev->region[i].ops->add_capability(
						cdev, &cdev->region[i], &caps);
				if (rc)
					return rc;
			}
		} /* default */
