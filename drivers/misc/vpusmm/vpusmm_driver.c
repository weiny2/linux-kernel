...
static dev_t vpusmm_device_number = 0;
static struct class *vpusmm_sys_class = NULL;
static int vpusmm_platform_driver_registerd = 0;
static struct platform_device* vpusmm_platform_dev = NULL;
...
// the life of the buffer may be longer than session(it may be shared with other driver),
// so it contains pointer to device rather than to session
struct vpusmm_buffer{
    struct device * dev;        // dma API device
    void * cookie;              // the kernel space virtual address of the DMA buffer
    dma_addr_t dma_addr;        // the dma address for current device
    size_t size;                // length of the buffer
    unsigned long dma_attrs;    // DMA attr
};

// DMABufs imported to VPU device are maintained in a rb tree with dmabuf's pointer as key.
// so user space can unimport it by specifying fd and ptr2vpu() API can work by searching
// this rb tree.
...
{
    if (sgt->nents >= 1) {
        /* check if the entries in the sg_table are contiguous and accessible by VPU
        */
        ...
        return (next_addr - 1) <= min_not_zero(dev->coherent_dma_mask, dev->bus_dma_limit);
    }
    return 0;
}
...
*/
static void vpusmm_session_insert_imp_dmabuf(struct vpusmm_session * sess, struct vpusmm_imported_dmabuf * new_item)
{
    ...
    while (*iter)
    {
        ...
        if (item->dmabuf == value){
            printk(KERN_ERR" vpusmm_session_insert_imp_dmabuf error: insert exist dmabuf!\n");
        }
        if (item->dmabuf > value)
            iter = &(*iter)->rb_left;
        else
...
static struct vpusmm_imported_dmabuf *vpusmm_session_find_imp_dmabuf(struct vpusmm_session * sess, struct dma_buf * dmabuf)
{
...
static struct vpusmm_imported_dmabuf * vpusmm_session_import_dmabuf(
    ...
    int vpu_id)
{
    ...
    /*
        the vpu_addr here needs special care, DMAbuf exporter's callback map_dma_buf()
        will give dma_address for our device usually.
        but in fake IOMMU cases, our device is just a fake platform device and dma_address
        is simply the physical address, we need to translate it into right dma_address given vpu_id.

        1.in accordance with Linux DMA framework, we should add iommus into vpusmm device tree node
          and map_dma_buf() will automatically setup the mapping and return dma_address.

        2.if use 1:1 mapping between vpusmm device and vpu slice, we should extract vpu_id from
            MINOR(sess->dev->device_number) rather than support any vpu_id.
    */
    item->dmabuf = dmabuf;
    item->attach = attach;
    item->sgt = sgt;
...
static void vpusmm_session_unimport_dmabuf(struct vpusmm_session * sess, struct dma_buf * dmabuf){
    struct vpusmm_imported_dmabuf * item = NULL;
...
/*======================================================================================================================
* driver's open/release fops, will maintain session concept here
*/
...
#if 1
    pr_info("[%s] current:0x%p PID:%d (%s)  PID2=%d, filp=%p open iminor=%d, f_flags=%#o (%s) f_count=%ld\n", __FUNCTION__,
        current, current->pid, current->comm, pid_nr(get_task_pid(current, PIDTYPE_PID)),
        filp,
                iminor(inode),
                filp->f_flags,
                filp->f_flags & O_APPEND ? "O_APPEND":"",
                atomic_long_fetch_add(0, &filp->f_count));
...
static int vpusmm_release(struct inode *inode, struct file *filp)
{
    ...
    pr_info("[%s] PID:%d filp=%p iminor=%d, release f_flags=%#o\n", __FUNCTION__, current->pid, filp, iminor(inode), filp->f_flags);
...
//  DMABuf allocator
//
static struct sg_table *vpusmm_map_dma_buf(struct dma_buf_attachment *attach, enum dma_data_direction dir)
{
...
static void vpusmm_unmap_dma_buf(struct dma_buf_attachment *attach,struct sg_table *sgt, enum dma_data_direction dir)
{
    if (sgt) {
        if (dir != DMA_NONE)
...
static int vpusmm_dmabuf_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
    ...
    //pr_info("[vpusmm]: vpusmm_dmabuf_mmap is called start=%lx, end=%lx-------------------\n", vma->vm_start, vma->vm_end);

    if(buff->dma_attrs & DMA_ATTR_NON_CONSISTENT){
        // in this case, user-space process will have a non-coherent mapping, but kernel space has
        // coherent mapping, not sure if it's a problem

#if USE_VMF == 1
        // following flags prevent VM subsystem from doing strange operations on this mapping
        vma->vm_flags |= VM_IO | VM_PFNMAP | VM_DONTEXPAND | VM_DONTDUMP;
        vma->vm_ops = &vpusmm_device_vm_ops;
#else
        unsigned long address;
        unsigned long phys_addr = dma_to_phys(buff->dev, buff->dma_addr);
        unsigned long pfn = phys_addr >> PAGE_SHIFT;
        // following flags prevent VM subsystem from doing strange operations on this mapping
        vma->vm_flags |= VM_IO | VM_PFNMAP | VM_DONTEXPAND | VM_DONTDUMP;

        //setup pfns
        for(address = vma->vm_start; address < vma->vm_end; address += PAGE_SIZE, pfn ++){
            vmf_insert_pfn(vma, address, pfn);
        }
#endif
    } else {
...
static int vpusmm_dmabuf_begin_cpu_access(struct dma_buf *dmabuf, enum dma_data_direction direction)
{
    ...
    if(buff->dma_attrs & DMA_ATTR_NON_CONSISTENT) {
       ...
       //pr_info("vpusmm_dmabuf_begin_cpu_access:  buff->size=%lu direction=%d\n", buff->size, direction);
    }
    return 0;
}
static int vpusmm_dmabuf_end_cpu_access(struct dma_buf *dmabuf, enum dma_data_direction  direction)
{
    ...
    if(buff->dma_attrs & DMA_ATTR_NON_CONSISTENT) {
       ...
       //pr_info("vpusmm_dmabuf_end_cpu_access:  buff->size=%lu direction=%d\n", buff->size, direction);
    }
    if(buff->dma_attrs & DMA_ATTR_WRITE_COMBINE) {
        // FLUSH write buffer, HOWTO?
    }
    return 0;
}
...
* export as DMABuf fd
* sess will hold additional refcount to the dmabuf on request of passing it to VPU side for processing
*/
static long vpusmm_session_alloc(struct vpusmm_session * sess, struct vpusmm_args_alloc * arg)
{
    ...
    buff->cookie = dma_alloc_attrs(dev->mem_dev[0], buff->size, &buff->dma_addr, GFP_KERNEL | GFP_DMA, buff->dma_attrs);
    if (buff->cookie == NULL) {
...
/**===================================================================================================
* to pass a DMABuf to VPU side for accessing, following general guideline of DMABuf
* we need to do attach & map & unmap & detach sequences, and in our case, this needs
* to be done in user-space for now.
*
* Note that we cannot presume the dmabuf VPU is about to access are from our allocator
* it may come from any device driver (codec,camera,...), as long as the backing pages
* are physical contiguous.
*
*   dma_addr = VPUSMM_VPU_map(dmabuf_fd, direction)
*   if(dma_addr){
*        //mapping failure, the buffer is cannot be acccessed by VPU directly
*    }
*
*   dma_addr can then be passed to VPU side as a meaningful address
*
*   VPUSMM_VPU_unmap(dmabuf_fd)
*
*   if a dmabuf has been mem-mapped into process's virtual address space
*   from vma we can locate the dmabuf, and if it was also mapped to VPU by us,
*   from that dmabuf object we should be able to locate it's sgt too,
*   so ptr-to-phys mapping is still viable, given that above map operation is done correctly.
*
*   VPUSMM_VPU_map() will maintain a map between dmabuf <==> vpusmm_dmabuf_map with the help of
*   some special data structure in kernel, so APP crash will free those maps after VPU side have
*   done their work, so this map itself should be ref-counted
*
*/

static long vpusmm_session_import(struct vpusmm_session * sess, struct vpusmm_args_import * arg)
{
    struct dma_buf * dmabuf = NULL;
    struct vpusmm_imported_dmabuf * item;
    enum dma_data_direction direction;
...
static long vpusmm_session_ptr2vpu(struct vpusmm_session * sess, unsigned long * arg)
{
    ...
    //pr_info("[vpusmm]: line %d, task->pid=%d, vaddr=%lx, mm=%px, active_mm=%px\n", __LINE__,task->pid, vaddr, task->mm, task->active_mm);
...
static long vpusmm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    if (_IOC_TYPE(cmd) != VPUSMM_IOC_MAGIC || _IOC_NR(cmd) >= _IOC_NR(VPUSMM_IOCTL_END))
        return -EINVAL;
...
static int vpusmm_probe(struct platform_device *pdev)
{
    ...
    if ((rc = cdev_add(&vpusmm_dev->cdev, vpusmm_dev->device_number, 1)) != 0) {
        goto err;
    }
    ...
    */
    ...
    if (vpusmm_dev->rmem_cnt <= 0) {
        ...
        for (i=0; i<vpusmm_dev->rmem_cnt; i++) {
            /* Create a child devices (of platform dev, not the cdev) to own the reserved memory. */
            ...
            mem_dev->bus_dma_limit = dev->bus_dma_limit;
...
err:
    dev_err(dev, "vpusmm_probe(%d) failed, return=%d\n",id, rc);
 ...
 */
static struct of_device_id vpusmm_of_match[] = {
...
static void vpusmm_cleanup(void)
{
    ...
    if (vpusmm_platform_driver_registerd){
...
static int __init  vpusmm_init(void)
{
    ...
    vpusmm_sys_class = class_create(THIS_MODULE, DRIVER_NAME"_class");
       if(IS_ERR_OR_NULL(vpusmm_sys_class)){
        ...
        vpusmm_sys_class = NULL;
        printk(KERN_ERR"[%s]: couldn't create driver class, return=%d\n", DRIVER_NAME, retval);
    ...
    if (np) {
        ...
        // There is no vpusmm node in device tree, just create one
        ...
        if (IS_ERR_OR_NULL(vpusmm_platform_dev)) {
            ...
            vpusmm_platform_dev   = NULL;
            printk(KERN_ERR "platform_device_register_full(%s) failed. return=%d\n", DRIVER_NAME, retval);
            goto failed;
        }
        pr_info("vpusmm_init: device tree node not found, create a default device\n");
    }

    retval = platform_driver_register(&vpusmm_platform_driver);
    if (retval) {
        printk(KERN_ERR "%s: couldn't register platform driver. return=%d\n", DRIVER_NAME, retval);
        vpusmm_platform_driver_registerd = 0;
        goto failed;
    } else {
        vpusmm_platform_driver_registerd = 1;
    }

    pr_info("vpusmm_init done\n");
