						 ...
						 bool is_ldb)
{
	...
	address = dlb2_dev->hw.func_phys_addr;
	...
	return devm_ioremap(&dlb2_dev->pdev->dev, address, size);
}
					  ...
					  void __iomem *addr)
{
	...
	devm_iounmap(&dlb2_dev->pdev->dev, addr);
}
...
static inline int os_notify_user_space(struct dlb2_hw *hw,
				       ...
				       u64 aux_alert_data)
{
	struct dlb2_domain_dev *domain_dev;
	struct dlb2_domain_alert alert;
	struct dlb2_dev *dlb2_dev;

	dlb2_dev = container_of(hw, struct dlb2_dev, hw);

	domain_dev = &dlb2_dev->sched_domains[domain_id];
...
/**
 * DLB2_BASE_DBG() - log a debug message
 * @dlb2: dlb2_hw handle for a particular device.
 * @...: variable string args.
 */
#define DLB2_BASE_DBG(dlb2, ...) do {		       \
	struct dlb2_dev *dev;			       \
	dev = container_of(dlb2, struct dlb2_dev, hw); \
...
static void dlb2_complete_queue_map_unmap(struct work_struct *work)
{
	...
	if (ret != 0)
		 ...
		 */
		queue_work(dlb2_dev->wq, &dlb2_dev->work);
	else
