 ...
 */
static struct dlb2_dev *dlb2_devices[DLB2_MAX_NUM_DEVICES];

void dlb2_datapath_init(struct dlb2_dev *dev, int id)
{
	dlb2_devices[id] = dev;
	      ...
	      struct dlb2_dp **hdl)
{
	...
	dlb2 = devm_kcalloc(&dev->pdev->dev,
			    1,
			    sizeof(struct dlb2_dp),
			    GFP_KERNEL);
	if (!dlb2) {
		ret = -ENOMEM;
		goto cleanup;
	...
	dlb2->dev = dev;
	dlb2->magic_num = DLB2_MAGIC_NUM;
	dlb2->id = device_id;

	mutex_init(&dlb2->resource_mutex);

	dlb2_register_dp_handle(dlb2);
...
cleanup:
	...
	if (ret && dlb2)
		devm_kfree(&dev->pdev->dev, dlb2);

	return ret;
}
EXPORT_SYMBOL(dlb2_open);

int dlb2_close(struct dlb2_dp *dlb2)
{
	int i, ret = -1;
 ...
 */
	...
	if (dlb2->magic_num != DLB2_MAGIC_NUM) {
		ret = -EINVAL;
		goto cleanup;
	}
#endif
	...
	for (i = 0; i < DLB2_MAX_NUM_DOMAINS; i++)
		if (dlb2->domains[i].configured &&
		    !list_empty(&dlb2->domains[i].hdl_list_head)) {
			ret = -EEXIST;
			goto cleanup;
		}
	...
	for (i = 0; i < DLB2_MAX_NUM_DOMAINS; i++)
		if (dlb2->domains[i].configured)
			dlb2_reset_sched_domain(dlb2, i);

	dlb2_unregister_dp_handle(dlb2);

	devm_kfree(&dlb2->dev->pdev->dev, dlb2);
...
static int dlb2_dp_ioctl_get_num_resources(struct dlb2_dp *dlb2,
					   struct dlb2_resources *rsrcs)
{
	struct dlb2_get_num_resources_args ioctl_args = {0};
	int ret;

	ret = dlb2_ioctl_get_num_resources(dlb2->dev,
...
int dlb2_get_num_resources(struct dlb2_dp *dlb2,
			   struct dlb2_resources *rsrcs)
{
	int ret = -1;
	...
	ret = dlb2_dp_ioctl_get_num_resources(dlb2, rsrcs);
...
static int dlb2_dp_ioctl_create_sch_dom(struct dlb2_dp *dlb2,
					struct dlb2_create_sched_domain *args)
{
	...
	ret = __dlb2_ioctl_create_sched_domain(dlb2->dev,
					       (unsigned long)&ioctl_args,
					       false,
					       dlb2);

	dlb2_log_ioctl_error(dlb2->dev->dlb2_device, ret, response.status);

	return (ret == 0) ? response.id : ret;
}

int dlb2_create_sched_domain(struct dlb2_dp *dlb2,
			     struct dlb2_create_sched_domain *args)
{
	struct dlb2_dp_domain *domain;
	int id, ret, i;
	...
	id = dlb2_dp_ioctl_create_sch_dom(dlb2, args);
	if (id < 0) {
		ret = id;
	...
	domain = &dlb2->domains[id];

	memset(domain, 0, sizeof(struct dlb2_dp_domain));
	...
	domain->status = domain->dev->sched_domains[id].status;
...
dlb2_attach_sched_domain(struct dlb2_dp *dlb2,
			 int domain_id)
{
	...
	if (!dlb2->domains[domain_id].configured) {
		ret = -EINVAL;
		goto cleanup;
	}

	domain = &dlb2->domains[domain_id];

	mutex_lock(&domain->resource_mutex);

	domain_hdl = devm_kcalloc(DEV_FROM_DLB2_DP_DOMAIN(domain),
				  1,
				  sizeof(struct dlb2_domain_hdl),
				  GFP_KERNEL);
	if (!domain_hdl) {
		ret = -ENOMEM;
...
int dlb2_detach_sched_domain(struct dlb2_domain_hdl *hdl)
{
	...
	memset(hdl, 0, sizeof(struct dlb2_domain_hdl));
					  ...
					  struct dlb2_create_ldb_queue *args)
{
	...
	ret = dlb2_domain_ioctl_create_ldb_queue(domain->dev,
						 (unsigned long)&ioctl_args,
						 domain->id);

	dlb2_log_ioctl_error(domain->dev->dlb2_device, ret, response.status);
					  ...
					  int port_id)
{
	...
	ret = dlb2_domain_ioctl_create_dir_queue(domain->dev,
						 (unsigned long)&ioctl_args,
						 domain->id);
		     ...
		     int port_id)
{
	...
	if (!(port_id >= 0 && port_id < DLB2_MAX_NUM_DIR_PORTS)) {
		ret = -EINVAL;
		goto cleanup;
	}
...
int dlb2_detach_port(struct dlb2_port_hdl *hdl)
{
	...
	memset(hdl, 0, sizeof(struct dlb2_port_hdl));
				  ...
				  int priority)
{
	...
	ret = dlb2_domain_ioctl_map_qid(domain->dev,
					(unsigned long)&ioctl_args,
					domain->id);
				    ...
				    int queue_id)
{
	...
	ret = dlb2_domain_ioctl_unmap_qid(domain->dev,
					  (unsigned long)&ioctl_args,
					  domain->id);
					 ...
					 int port_id)
{
	...
	ret = dlb2_domain_ioctl_enable_ldb_port(domain->dev,
						(unsigned long)&ioctl_args,
						domain->id);
					 ...
					 int port_id)
{
	...
	ret = dlb2_domain_ioctl_enable_dir_port(domain->dev,
						(unsigned long)&ioctl_args,
						domain->id);
					  ...
					  int port_id)
{
	...
	ret = dlb2_domain_ioctl_disable_ldb_port(domain->dev,
						 (unsigned long)&ioctl_args,
						 domain->id);
					  ...
					  int port_id)
{
	...
	ret = dlb2_domain_ioctl_disable_dir_port(domain->dev,
						 (unsigned long)&ioctl_args,
						 domain->id);
...
int dlb2_dp_ioctl_start_domain(struct dlb2_dp_domain *domain)
{
	...
	ret = dlb2_domain_ioctl_start_domain(domain->dev,
					     (unsigned long)&ioctl_args,
					     domain->id);
					      ...
					      u64 aux_alert_data)
{
	...
	ret = dlb2_domain_ioctl_enqueue_domain_alert(domain->dev,
						     args_ptr,
						     domain->id);
...
int dlb2_reset_sched_domain(struct dlb2_dp *dlb2,
			    int domain_id)
{
	struct dlb2_dp_domain *domain;
	struct device *device;
	...
	for (i = 0; i < DLB2_MAX_NUM_LDB_PORTS; i++) {
		struct dlb2_port *port = &domain->ldb_ports[i];

		if (port->configured)
			devm_iounmap(domain->dev->dlb2_device,
				     port->pp_addr);
		memset(port, 0, sizeof(struct dlb2_port));
	}

	for (i = 0; i < DLB2_MAX_NUM_DIR_PORTS; i++) {
		struct dlb2_port *port = &domain->dir_ports[i];

		if (port->configured)
			devm_iounmap(domain->dev->dlb2_device, port->pp_addr);
	 ...
	 * resets the domain.
	 */
	mutex_lock(&dlb2->dev->resource_mutex);

	ret = dlb2_close_domain_device_file(domain->dev,
					    domain->status,
					    domain->id,
					    domain->shutdown);

	mutex_unlock(&dlb2->dev->resource_mutex);
					...
					struct dlb2_dp_domain_alert *alert)
{
	...
	ret = dlb2_read_domain_alert(domain->dev,
				     domain->status,
				     domain->id,
				     &kernel_alert,
				     false);
	...
	default:
		if (alert_id < NUM_DLB2_DOMAIN_ALERTS)
			dev_err(domain->dev->dlb2_device,
...
static inline void dec_port_owed_releases(struct dlb2_port *port,
...
static inline void inc_port_owed_releases(struct dlb2_port *port,
					  int cnt)
{
	port->owed_releases += cnt;
}

static inline void dec_port_owed_tokens(struct dlb2_port *port,
					struct dlb2_enqueue_qe *enqueue_qe)
{
	enum dlb2_event_cmd_t cmd = enqueue_qe->cmd_info.qe_cmd;
...
static inline void inc_port_owed_tokens(struct dlb2_port *port, int cnt)
{
	port->owed_tokens += cnt;
}

static inline void release_port_credits(struct dlb2_port *port)
{
...
static inline void refresh_port_credits(struct dlb2_port *port,
...
static inline void inc_port_credits(struct dlb2_port *port, int num)
{
					    ...
					    struct dlb2_port *port)
{
	...
	memset(enqueue_qe, 0, sizeof(struct dlb2_enqueue_qe) * 4);
					       ...
					       bool arm)
{
	...
	ret = dlb2_domain_ioctl_block_on_cq_interrupt(domain->dev,
						      args_ptr,
						      domain->id);
...
static inline bool port_cq_is_empty(struct dlb2_port *port)
{
...
static int map_consumer_queue(struct dlb2_dev *dev, struct dlb2_port *port)
{
	if (port->type == LDB)
		port->cq_base = dev->ldb_port_mem[port->id].cq_base;
	else
		port->cq_base = dev->dir_port_mem[port->id].cq_base;

	return (!port->cq_base) ? -1 : 0;
}

static int map_producer_port(struct dlb2_dev *dev, struct dlb2_port *port)
{
	port->pp_addr = devm_ioremap_wc(dev->dlb2_device,
...
int dlb2_dp_ioctl_create_ldb_port(struct dlb2_dp_domain *domain,
				  struct dlb2_create_port *args,
				  struct dlb2_create_port_adv *adv_args)
{
	...
	ret = dlb2_domain_ioctl_create_ldb_port(domain->dev,
						(unsigned long)&ioctl_args,
						domain->id);
...
int dlb2_dp_ioctl_create_dir_port(struct dlb2_dp_domain *domain,
				  struct dlb2_create_port *args,
				  struct dlb2_create_port_adv *adv_args,
				  int queue_id)
{
	...
	ret = dlb2_domain_ioctl_create_dir_port(domain->dev,
						(unsigned long)&ioctl_args,
						domain->id);
...
int dlb2_adv_send(struct dlb2_port_hdl *hdl,
		  u32 num,
		  union dlb2_event *evts,
		  int *error)
{
	 ...
	 * events *may* need credits, so __dlb2_adv_send() needs to check every
	 * event's cmd.
	 */
	ret = __dlb2_adv_send(hdl, num, evts, error, true, false);
cleanup:
