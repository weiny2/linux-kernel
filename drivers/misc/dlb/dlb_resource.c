...
int dlb_resource_init(struct dlb_hw *hw)
{
	...
	for (i = 0; i < hw->pf.num_avail_domains; i++) {
		...
		list_add(&hw->pf.avail_domains, list);
	}
	...
	for (i = 0; i < hw->pf.num_avail_ldb_queues; i++) {
		...
		list_add(&hw->pf.avail_ldb_queues, list);
	}
	...
	for (i = 0; i < hw->pf.num_avail_ldb_ports; i++) {
		...
		list_add(&hw->pf.avail_ldb_ports, &port->func_list);
	}
	...
	for (i = 0; i < hw->pf.num_avail_dir_pq_pairs; i++) {
		...
		list_add(&hw->pf.avail_dir_pq_pairs, list);
	}
	...
	for (i = 0; i < hw->pf.num_avail_ldb_credit_pools; i++) {
		...
		list_add(&hw->pf.avail_ldb_credit_pools, list);
	}
...
#define DLB_XFER_LL_RSRC(dst, src, num, type_t, name) ({	    \
	...
	DLB_FUNC_LIST_FOR_SAFE(_src->avail_##name##s, ptr, tmp) {   \
		...
		list_add(&_dst->avail_##name##s,  &ptr->func_list); \
				 ...
				 struct dlb_cmd_response *resp)
{
	...
	for (i = 0; i < num_queues; i++) {
		...
		list_add(&domain->avail_ldb_queues, &queue->domain_list);
	}
...
cleanup:
	...
	for (j = 0; j < i; j++) {
		...
		list_add(&rsrcs->avail_ldb_queues, &queue->func_list);
	}
				...
				struct dlb_cmd_response *resp)
{
	...
	for (i = 0; i < num_ports; i++) {
		...
		list_add(&domain->avail_ldb_ports, &port->domain_list);
	}
...
cleanup:
	...
	for (j = 0; j < i; j++) {
		...
		list_add(&rsrcs->avail_ldb_ports, &port->func_list);
	}
				...
				struct dlb_cmd_response *resp)
{
	...
	for (i = 0; i < num_ports; i++) {
		...
		list_add(&domain->avail_dir_pq_pairs, &port->domain_list);
	}
...
cleanup:
	...
	for (j = 0; j < i; j++) {
		...
		list_add(&rsrcs->avail_dir_pq_pairs, &port->func_list);
	}
				       ...
				       struct dlb_cmd_response *resp)
{
	...
	for (i = 0; i < num_credit_pools; i++) {
		...
		list_add(&domain->avail_ldb_credit_pools, &pool->domain_list);
	}
...
cleanup:
	...
	for (j = 0; j < i; j++) {
		...
		list_add(&rsrcs->avail_ldb_credit_pools, &pool->func_list);
	}
				       ...
				       struct dlb_cmd_response *resp)
{
	...
	for (i = 0; i < num_credit_pools; i++) {
		...
		list_add(&domain->avail_dir_credit_pools, &pool->domain_list);
	}
...
cleanup:
	...
	for (j = 0; j < i; j++) {
		...
		list_add(&rsrcs->avail_dir_credit_pools, &pool->func_list);
	}
				     ...
				     unsigned int vf_id)
{
	...
	r10.field.enab_shallow_cq = args->cq_depth < 8;
			       ...
			       unsigned int vf_id)
{
	...
	list_add(&rsrcs->used_domains, &domain->func_list);
			   ...
			   unsigned int vf_id)
{
	...
	list_add(&domain->used_ldb_credit_pools, &pool->domain_list);
			   ...
			   unsigned int vf_id)
{
	...
	list_add(&domain->used_dir_credit_pools, &pool->domain_list);
			    ...
			    unsigned int vf_id)
{
	...
	list_add(&domain->used_ldb_queues, &queue->domain_list);
			    ...
			    unsigned int vf_id)
{
	...
	if (args->port_id == -1) {
		...
		list_add(&domain->used_dir_pq_pairs, &queue->domain_list);
	}
					 ...
					 unsigned int vf_id)
{
	...
	DLB_HW_DBG(hw, "\tpop_count_address:         0x%lx\n",
		   pop_count_dma_base);
			   ...
			   unsigned int vf_id)
{
	...
	list_add(&domain->used_ldb_ports, &port->domain_list);
			   ...
			   unsigned int vf_id)
{
	...
	if (args->queue_id == -1) {
		...
		list_add(&domain->used_dir_pq_pairs, &port->domain_list);
	}
					   ...
					   struct dlb_domain *domain)
{
	...
	DLB_DOM_LIST_FOR_SAFE(*list, ldb_queue, tmp_ldb_queue) {
		...
		list_add(&rsrcs->avail_ldb_queues, &ldb_queue->func_list);
	...
	DLB_DOM_LIST_FOR_SAFE(*list, ldb_port, tmp_ldb_port) {
		...
		list_add(&rsrcs->avail_ldb_ports, &ldb_port->func_list);
	...
	DLB_DOM_LIST_FOR_SAFE(*list, dir_port, tmp_dir_port) {
		...
		list_add(&rsrcs->avail_dir_pq_pairs, &dir_port->func_list);
	 ...
	 */
	list_del(&domain->func_list);
	list_add(&rsrcs->avail_domains, &domain->func_list);
	rsrcs->num_avail_domains++;
