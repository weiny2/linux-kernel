				   ...
				   u32 reg, size_t size, u32 val)
{
	...
	{
		if ((reg == PCI_BASE_ADDRESS_0) || (reg == PCI_BASE_ADDRESS_2) || (reg == PCI_BASE_ADDRESS_4))
			return;
	}
...
static void thunderbay_pcie_ep_init(struct dw_pcie_ep *ep)
{
	...
	static struct resource res2[8];
	static struct resource res4[8];
	int ret;
	int func_idx;
	ret = of_property_read_u8(pci->dev->of_node, "max-functions", &ep->epc->max_functions);
	if (ret < 0)
	ep->epc->max_functions = 1;
	for (func_idx = 0;func_idx < ep->epc->max_functions;func_idx++)
	{
		np = of_parse_phandle(pci->dev->of_node, "memory-region", func_idx*2);
		if (!np) {
			dev_err(dev, "No %s for BAR2 specified in device tree\n", "memory-region");
			return;
		}
		ret = of_address_to_resource(np, 0, &res2[func_idx]);
		if (ret) {
			dev_err(dev,"No memory address assigned to the region\n");
			return;
		}
		thunderbay_pcie->mmr2[func_idx] = &res2[func_idx];
		printk("thunderbay->mmr2[%d]->start=%llx\n",func_idx,thunderbay_pcie->mmr2[func_idx]->start);
		np = of_parse_phandle(pci->dev->of_node, "memory-region", (func_idx*2)+1);
		if (!np) {
			dev_err(dev,"No %s for BAR4 specified in device tree\n", "memory-region");
			return;
		}
		ret = of_address_to_resource(np, 0, &res4[func_idx]);
		if (ret) {
			dev_err(dev,"No memory address assigned to the region\n");
			return;
		}
		thunderbay_pcie->mmr4[func_idx] = &res4[func_idx];
		printk("thunderbay->mmr4[%d]->start=%llx\n",func_idx,thunderbay_pcie->mmr4[func_idx]->start);
		thunderbay_pcie->setup_bar[func_idx] = false;
		thunderbay_pcie->irq_doorbell[func_idx] = irq_of_parse_and_map(pci->dev->of_node, func_idx+2);
		printk("thunderbay->irq_doorbell[%d]=%d\n",func_idx,thunderbay_pcie->irq_doorbell[func_idx]);
		thunderbay_pcie->irq_wdma[func_idx] = irq_of_parse_and_map(pci->dev->of_node, func_idx+10);
		printk("thunderbay->irq_wdma[%d]=%d\n",func_idx,thunderbay_pcie->irq_wdma[func_idx]);
		thunderbay_pcie->irq_rdma[func_idx] = irq_of_parse_and_map(pci->dev->of_node, func_idx+18);
		printk("thunderbay->irq_rdma[%d]=%d\n",func_idx,thunderbay_pcie->irq_rdma[func_idx]);
	}
#if 0
...
static int __init thunderbay_pcie_probe(struct platform_device *pdev)
{
	...
	pcie->doorbell_base = platform_get_resource_byname(pdev, IORESOURCE_MEM, "doorbell");
	dev_dbg(pci->dev, "pcie->doorbell_base=0x%llx\n",pcie->doorbell_base->start);
	if (IS_ERR(pcie->doorbell_base))
		return PTR_ERR(pcie->doorbell_base);
	pcie->doorbell_clear = platform_get_resource_byname(pdev, IORESOURCE_MEM, "doorbellclr");
	dev_dbg(pci->dev, "pcie->doorbell_clear=0x%llx\n",pcie->doorbell_clear->start);
	if (IS_ERR(pcie->doorbell_clear))
		return PTR_ERR(pcie->doorbell_clear);
...
//	printk("pcie:pci->dbi_base=%llx\n",virt_to_phys(pci->dbi_base));
//	printk("pcie:pci->dbi_base2=%llx\n",virt_to_phys(pci->dbi_base2));

	switch (pcie->mode) {
	case DW_PCIE_RC_TYPE:
		if (!IS_ENABLED(CONFIG_PCIE_TBH_HOST))
			return -ENODEV;
