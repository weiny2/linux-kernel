...
/*!
 *  @brief HQM event send structure
 */
struct hqmv2_send {
	...
	u8 queue_id;
	/* Scheduling type (use enum hqmv2_event_sched_t) */
	u8 sched_type:2;
	/* Priority */
	u8 priority:3;
	/* Reserved */
...
/*!
 *  @brief HQM event receive structure
 */
struct hqmv2_recv {
	/* 64-bit event data */
	u64 udata64;
	...
	u8 queue_id;
	/* Scheduling type (enum hqmv2_event_sched_t) */
	u8 sched_type:2;
	/* Priority */
	u8 priority:3;
	/* Reserved */
...
/*!
 *  @brief Advanced HQM event send structure
 */
struct hqmv2_adv_send {
	/* 64-bit event data */
	u64 udata64;
	/* 16 bits of user data */
	u16 udata16;
	...
	/** Reserved */
	u8 rsvd1:7;
	/** Send command (use enum hqmv2_event_cmd_t) */
	u8 cmd:4;
	/** Reserved */
	u8 rsvd2:4;
} __packed;
...
/*!
 *  @brief HQM event structure
 */
union hqmv2_event {
	/* Structure for sending events */
	struct hqmv2_send send;
	/* Structure for receiving events */
	struct hqmv2_recv recv;
	/* Structure for sending events with the advanced send function */
	struct hqmv2_adv_send adv_send;
};

/******************************/
/* HQM Device-Level Functions */
/******************************/

/*!
 * @fn int hqmv2_open(
 *          int device_id,
 *          struct hqmv2_dp **hqmv2);
 *
 * @brief   Open the HQM device file and initialize the client library.
 *
 * @note    An HQM handle can be shared among kernel threads. Functions that
 *          take an HQM handle are MT-safe, unless otherwise noted. When
 *          hqmv2_close() is called for a particular HQM handle, that handle can
 *          no longer be used.
 *
 * @param[in]  device_id: The ID of an HQM device file. That is, N in /dev/hqmN.
 * @param[out] hdl: Pointer to memory where an HQM handle will be stored.
 *
 * @retval  0 Success, hdl points to a valid HQM handle
 * @retval -ENOENT No HQM device found
 * @retval -EINVAL Invalid device ID or hdl pointer
 * @retval -EINVAL Client library version is incompatible with the installed
 *                   HQM driver
 */
int
hqmv2_open(int device_id,
	   struct hqmv2_dp **hqmv2);
 ...
 *          struct hqmv2_dp *hqmv2);
 *
 * @brief   Clean up the client library and close the HQM device file
 *          associated with the HQM handle. The user must detach all scheduling
 *          domain handles attached with this handle before calling this
 *          function, else it will fail.
 *
 * @param[in] hqmv2: Handle returned by a successful call to hqmv2_open().
 *
 * @retval  0 Success
 * @retval -EEXIST There is at least one remaining attached domain handle
 * @retval -EINVAL Invalid HQM handle
 */
int
hqmv2_close(struct hqmv2_dp *hqmv2);

/*!
 *  @brief HQM resources
 */
struct hqmv2_resources {
	 ...
	 * deadlock (@see hqmv2_send for details on avoiding deadlock).
	 */
	 ...
	 * hqmv2_send for details on avoiding deadlock).
	 */
	u32 num_dir_credit_pools;
};

/*!
 * @fn int hqmv2_get_num_resources(
 *          struct hqmv2_dp *hqmv2,
 *          struct hqmv2_resources *rsrcs);
 *
 * @brief   Get the current number of available HQM resources. These
 *          resources can be assigned to a scheduling domain with
 *          hqmv2_create_sched_domain() (if there are any available domains).
 *
 * @param[in]  hqmv2: Handle returned by a successful call to hqmv2_open().
 ...
 *
 * @retval  0 Success
 * @retval -EINVAL Invalid HQM handle or rsrcs pointer
 */
int
hqmv2_get_num_resources(struct hqmv2_dp *hqmv2,
			struct hqmv2_resources *rsrcs);
...
/*!
 *  @brief HQM scheduling domain creation arguments
 *
 *  @see struct hqmv2_resources
 */
struct hqmv2_create_sched_domain {
...
/*!
 * @fn int hqmv2_create_sched_domain(
 *          struct hqmv2_dp *hqmv2,
 ...
 *
 * @param[in] hqmv2: Handle returned by a successful call to hqmv2_open().
 * @param[in] args: Pointer to struct hqmv2_create_sched_domain structure.
 *
 * @retval >=0 Scheduling domain ID
 * @retval -EINVAL Insufficient HQM resources to satisfy the request
 */
int
hqmv2_create_sched_domain(struct hqmv2_dp *hqmv2,
			  struct hqmv2_create_sched_domain *args);

/*!
 * @fn struct hqmv2_domain_hdl *hqmv2_attach_sched_domain(
 *                          struct hqmv2_dp *hqmv2,
 *                          int domain_id);
 *
 ...
 */
int
hqmv2_detach_sched_domain(struct hqmv2_domain_hdl *hdl);

/*!
 * @fn int hqmv2_create_ldb_pool(
 *          struct hqmv2_domain_hdl *hdl,
 *          int num_credits);
 *
 ...
 * @retval -EINTR The HQM was unexpectedly reset. User must detach ports
 *		  then detach and reset scheduling domains, then can
 *		  attempt to re-create their scheduling domain(s).
 */
int
hqmv2_create_ldb_credit_pool(struct hqmv2_domain_hdl *hdl,
			     int num_credits);

/*!
 * @fn int hqmv2_create_dir_pool(
 *          struct hqmv2_domain_hdl *hdl,
 *          int num_credits);
 *
 ...
 */
int
hqmv2_create_dir_credit_pool(struct hqmv2_domain_hdl *hdl,
			     int num_credits);
 ...
 * HQM has 4 groups of ordered queues, where each group is configured to
 * contain either 1 queue with 1024 reorder entries, 2 queues with 512 reorder
 * entries, and so on down to 32 queues with 32 entries. These groups are
 ...
 * configuration will fail. All four queue groups default to 32 queues with 32
 * reorder entries, and these groups can be configured through the HQM sysfs
 * interface.
 *
 * (TODO: Add an ioctl to control this)
 */
struct hqmv2_create_ldb_queue {
	/**
 ...
 */
int
hqmv2_create_ldb_queue(struct hqmv2_domain_hdl *hdl,
		       struct hqmv2_create_ldb_queue *args);

/*!
 * @fn int hqmv2_create_dir_queue(
 *          struct hqmv2_domain_hdl *hdl,
 *          int port_id);
 *
 ...
 */
int
hqmv2_create_dir_queue(struct hqmv2_domain_hdl *hdl,
		       int port_id);

struct hqmv2_create_port {
 ...
 */
int
hqmv2_create_ldb_port(struct hqmv2_domain_hdl *hdl,
		      struct hqmv2_create_port *args);

/*!
 * @fn int hqmv2_create_dir_port(
 *          struct hqmv2_domain_hdl *hdl,
 *          struct hqmv2_create_port *args,
 *          int queue_id);
 *
 ...
 */
int
hqmv2_create_dir_port(struct hqmv2_domain_hdl *hdl,
		      struct hqmv2_create_port *args,
		      int queue_id);

/*!
 * @fn struct hqmv2_port_hdl *hqmv2_attach_ldb_port(
 *          struct hqmv2_domain_hdl *hdl,
 *          int port_id);
 *
 ...
 *          hqmv2_send and hqmv2_recv()) with the same port handle. Multiple
...
/*!
 * @fn struct hqmv2_port_hdl *hqmv2_attach_dir_port(
 *          struct hqmv2_domain_hdl *hdl,
 *          int port_id);
 *
...
/*!
 * @fn int hqmv2_detach_port(
 *          struct hqmv2_port_hdl *hdl);
 *
 * @brief   Detach a previously attached port handle.
 *
 * @param[in] port: Port handle returned by a successful call to
 *                  hqmv2_attach_ldb_port() or hqmv2_attach_dir_port().
 *
 * @retval  0 Success
 * @retval -EINVAL Invalid port handle
 */
int
hqmv2_detach_port(struct hqmv2_port_hdl *hdl);

/*!
 * @fn int hqmv2_link_queue(
 *          struct hqmv2_port_hdl *hdl,
 *          int queue_id,
 *          int priority);
 *
 ...
 */
int
hqmv2_link_queue(struct hqmv2_port_hdl *hdl,
		 int queue_id,
		 int priority);

/*!
 * @fn int hqmv2_unlink_queue(
 *          struct hqmv2_port_hdl *hdl,
 *          int queue_id);
 *
 ...
 */
int
hqmv2_unlink_queue(struct hqmv2_port_hdl *hdl,
		   int queue_id);

/*!
 * @fn int hqmv2_enable_port(
 *          struct hqmv2_port_hdl *hdl);
 *
 ...
 */
int
hqmv2_enable_port(struct hqmv2_port_hdl *hdl);

/*!
 * @fn int hqmv2_disable_port(
 *          struct hqmv2_port_hdl *hdl);
 *
 ...
 *          any events remaining in the port's CQ, but the HQM will not
 *          schedule any further events to the port until it is re-enabled.
 *
 * @note    If a thread is blocked in an HQM receive function call when the
 ...
 */
int
hqmv2_disable_port(struct hqmv2_port_hdl *hdl);

/*!
 * @fn int hqmv2_start_sched_domain(
 *          hqmv2_domain_hdl *hdl);
 *
 * @brief   This function is called to indicated the end of the HQM
 ...
 *		  must call hqmv2_launch_domain_alert_thread() before starting
 *		  the domain.
 *
 * @see hqmv2_launch_domain_alert_thread()
 */
int
hqmv2_start_sched_domain(struct hqmv2_domain_hdl *hdl);

/*!
 * @fn int hqmv2_reset_sched_domain(
 *          struct hqmv2_dp *hqmv2,
 *          int domain_id);
 *
 * @brief   This function resets a configured scheduling domain, effectively
 *          undoing the effects of hqmv2_create_sched_domain(). The domain's
 *          resources (ports, queues, pools, etc.) are relinquished to the HQM
 ...
 */
int
hqmv2_reset_sched_domain(struct hqmv2_dp *hqmv2,
			 int domain_id);
...
enum hqmv2_alert_id {
	 ...
	 * needs to detach all its handles and call hqmv2_close(), or simply
	 * exit and restart. The alert data doesn't contain any information for
	 * this alert.
	 */
	HQMV2_ALERT_DEVICE_RESET,
	/** The domain is being reset, triggered by a call to
	 * hqmv2_reset_sched_domain(). The application doesn't need to take any
	 * action in response to this alert. The alert data doesn't contain any
	 * information for this alert.
	 */
	HQMV2_ALERT_DOMAIN_RESET,
};

/*!
 *  @brief HQM alert information
 */
struct hqmv2_dp_domain_alert {
	enum hqmv2_alert_id id;
	u64 data;
};

/*!
 * @fn int hqmv2_launch_domain_alert_thread(
 *	    struct hqmv2_domain_hdl *hdl,
 *	    void (*callback)(struct hqmv2_dp_domain_alert *alert,
 *			     int domain_id,
 *			     void *arg),
 *	    void *callback_arg)
 *
 ...
 *
 *          Each kernel hqmv2 user must launch a domain alert thread before
 ...
 *
 * @see enum hqmv2_alert_id
 */
int
hqmv2_launch_domain_alert_thread(struct hqmv2_domain_hdl *hdl,
				 void (*cb)(struct hqmv2_dp_domain_alert *alert,
					    int domain_id,
					    void *arg),
				 void *cb_arg);
 ...
 * @param[in] events: Pointer to an array of hqmv2_event structures.
 ...
 * @exception -EINTR  The HQM was unexpectedly reset. User must detach ports
 *		      then detach and reset scheduling domains, then can
 *		      attempt to re-create their scheduling domain(s).
 */
int
hqmv2_send(struct hqmv2_port_hdl *hdl,
	   u32 num,
	   union hqmv2_event *events,
	   int *error);

/*!
 * @fn int hqmv2_release(
 *          struct hqmv2_port_hdl *hdl,
 *          u32 num,
 *          int *error);
 *
 * @brief   Release one or more events. Every event received by a
 *          load-balanced port has to be released; this tells the HQM that the
 ...
 */
int
hqmv2_release(struct hqmv2_port_hdl *hdl,
	      u32 num,
	      int *error);

/*!
 * @fn int hqmv2_forward(
 *          struct hqmv2_port_hdl *hdl,
 *          u32 num,
 *          union hqmv2_event *events,
 *          int *error);
 ...
 */
int
hqmv2_forward(struct hqmv2_port_hdl *hdl,
	      u32 num,
	      union hqmv2_event *events,
	      int *error);
 ...
 *
 * @note    hqmv2_recv() automatically pops the CQ for every received event,
 *          but hqmv2_recv_no_pop() does not. hqmv2_pop_cq() should only be
 *          used in conjunction with hqmv2_recv_no_pop().
 *
 * @note    This function is not MT-safe.
 *
 * @param[in] port: Port handle returned by a successful call to
 ...
 */
int
hqmv2_pop_cq(struct hqmv2_port_hdl *hdl,
	     u32 num,
	     int *error);
 ...
 *          hqmv2_release() or hqmv2_forward().
 ...
 *                  CQ is empty.
 * @param[out] events: Pointer to an array of hqmv2_event structures.
 * @param[out] error: If an error is detected, this is set according to the
 *		      following exception values. Otherwise, it is set to 0.
 *                    If NULL, the function will not record an error value.
 *
 ...
 */
int
hqmv2_recv(struct hqmv2_port_hdl *hdl,
	   u32 max,
	   bool wait,
	   union hqmv2_event *events,
	   int *err);
 ...
 *          hqmv2_recv_no_pop() does not automatically pop the event; the user
 *          must call hqmv2_pop_cq() to do so. Alternatively, the advanced send
 ...
 */
int
hqmv2_recv_no_pop(struct hqmv2_port_hdl *hdl,
		  u32 max,
		  bool wait,
		  union hqmv2_event *events,
		  int *err);
