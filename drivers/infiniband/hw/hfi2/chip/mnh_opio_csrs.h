...
typedef union {
    struct {
        ...
        uint64_t           Unused_3_1  :  3; // 
        uint64_t                 LANE  :  2; // Use this lane # to reset down stream framing
        uint64_t          Unused_63_6  : 58; // 
    } field;
    uint64_t val;
...
typedef union {
    struct {
        uint64_t                  VAL  :  4; // MNH will stay in Active mode. Link Layer Request to Physical Layer to request state change. Encodings as follows: 0000: NOP 0001: Active 0010: Reserved 0011: Deepest Allowable PM State [L1 Substates only] 0100: L1.1 0101: L1.2 0110: L1.3 0111: L1.4 1000: L2 1001: LinkReset 1010: Reserved 1011: Retrain 1100: Disable All other encodings are reserved. Please refer to the corresponding PHY HAS for state mapping and transition details.
...
typedef union {
    struct {
        ...
        uint64_t   LPHY_PRIM_RST_B_12  :  1; // #4 SIP opi_prim_rst_b Chassis Prim reset
        ...
        uint64_t      LPHY_VINF_ISO_B  :  1; // 
        uint64_t      LPHY_VCFN_ISO_B  :  1; // 
        uint64_t         Unused_31_26  :  6; // Unused
        uint64_t APHY_IOSF_SB_PWRGOOD  :  1; // HIP iopi_vccsyncsb_powergood
        ...
        uint64_t   APHY_IOSF_SB_RST_B  :  1; // HIP iopi_side_reset_b
        ...
        uint64_t   APHY_LANE_RST_B_61  :  1; // HIP iopi_lane_reset_b Data/Valid/ECC/Strobe
        uint64_t         Unused_63_62  :  2; // Unused
    } field;
    uint64_t val;
} OPIO_PHY_CFG_RESET_t;

// OPIO_PHY_CFG_RESETA desc:
typedef union {
    struct {
        uint64_t           Unused_7_0  :  8; // Unused
        uint64_t          APHY_PLL_EN  :  1; // PLL Enable
        uint64_t          Unused_63_9  : 55; // Unused
    } field;
    uint64_t val;
} OPIO_PHY_CFG_RESETA_t;

// OPIO_PHY_CFG_SW_CTRL desc:
typedef union {
    struct {
        uint64_t                   EN  :  1; // Gives reset control to OPIO_PHY_CFG_RESET
        uint64_t          Unused_63_1  : 63; // Unused
    } field;
    uint64_t val;
} OPIO_PHY_CFG_SW_CTRL_t;
...
typedef union {
    struct {
        uint64_t                  VAL  :  4; // Link Layer Request to Physical Layer to request state change. Encodings as follows: 0000: NOP 0001: Active 0010: Reserved 0011: Deepest Allowable PM State [L1 Substates only] 0100: L1.1 0101: L1.2 0110: L1.3 0111: L1.4 1000: L2 1001: LinkReset 1010: Reserved 1011: Retrain 1100: Disable All other encodings are reserved. Please refer to the corresponding PHY HAS for state mapping and transition details.
        uint64_t          Unused_63_4  : 60; // Unused
    } field;
    uint64_t val;
...
typedef union {
    struct {
        uint64_t                  VAL  :  2; // Indicates which valid vs. Idle data. Valid deasserted indicates idle data. This is used by MNH configuration only where LPIF_TXVALID_WIDTH==2 and this must match with NCLUSTER value.
        uint64_t          Unused_63_2  : 62; // Unused
    } field;
    uint64_t val;
