...
typedef union {
    struct {
        uint64_t leak_amount_fractional  : 16; // Fractional portion of leak amount
        uint64_t leak_amount_integral  :  3; // Integral portion of leak amount
        uint64_t       Reserved_30_19  : 12; // Unused
...
// TXDMA_ERR_INFO_MEM_RSP_STATUS desc:
typedef union {
    struct {
...
typedef union {
    struct {
        uint64_t      mc0_tc0_credits  :  4; // Credits available for MC0 TC0
        uint64_t      mc0_tc1_credits  :  4; // Credits available for MC0 TC1
        uint64_t      mc0_tc2_credits  :  4; // Credits available for MC0 TC2
        uint64_t      mc0_tc3_credits  :  4; // Credits available for MC0 TC3
        uint64_t      mc1_tc0_credits  :  4; // Credits available for MC1 TC0
        uint64_t      mc1_tc1_credits  :  4; // Credits available for MC1 TC1
        uint64_t      mc1_tc2_credits  :  4; // Credits available for MC1 TC2
        uint64_t      mc1_tc3_credits  :  4; // Credits available for MC1 TC3
        uint64_t       Reserved_63_32  : 32; // Unused
    } field;
    uint64_t val;
...
// TXDMA_DBG_TX_FLITS_CNTR_CTL desc:
typedef union {
    struct {
        uint64_t               tcmc_w  :  3; // The MC and TC selected for the TXDMA_TX_FLITS_TCMC_W performance counter. Upper bit is the MC and lower two bits are the TC.
        uint64_t               tcmc_x  :  3; // The MC and TC selected for the TXDMA_TX_FLITS_TCMC_X performance counter. Upper bit is the MC and lower two bits are the TC.
        uint64_t               tcmc_y  :  3; // The MC and TC selected for the TXDMA_TX_FLITS_TCMC_Y performance counter. Upper bit is the MC and lower two bits are the TC.
        uint64_t               tcmc_z  :  3; // The MC and TC selected for the TXDMA_TX_FLITS_TCMC_Z performance counter. Upper bit is the MC and lower two bits are the TC.
        uint64_t       Reserved_63_12  : 52; // Unused
    } field;
    uint64_t val;
} TXDMA_DBG_TX_FLITS_CNTR_CTL_t;

// TXDMA_DBG_STALL_EMPTY_CNTR_CTL desc:
typedef union {
    struct {
        uint64_t               tcmc_x  :  3; // The MC and TC selected for the TXDMA_STALL_EMPTY_EVENTS_X performance counter. Upper bit is the MC and lower two bits are the TC.
        uint64_t               tcmc_y  :  3; // The MC and TC selected for the TXDMA_STALL_EMPTY_EVENTS_Y performance counter. Upper bit is the MC and lower two bits are the TC.
        uint64_t        Reserved_63_6  : 58; // Unused
    } field;
    uint64_t val;
} TXDMA_DBG_STALL_EMPTY_CNTR_CTL_t;

// TXDMA_DBG_STALL_CREDITS_CNTR_CTL desc:
typedef union {
    struct {
        uint64_t               tcmc_x  :  3; // The MC and TC selected for the TXDMA_STALL_CREDITS_X performance counter. Upper bit is the MC and lower two bits are the TC.
        uint64_t               tcmc_y  :  3; // The MC and TC selected for the TXDMA_STALL_CREDITS_Y performance counter. Upper bit is the MC and lower two bits are the TC.
        uint64_t        Reserved_63_6  : 58; // Unused
    } field;
    uint64_t val;
} TXDMA_DBG_STALL_CREDITS_CNTR_CTL_t;

// TXDMA_DBG_MC0_0_PKT_CNTR_FLIT_MATCH desc:
typedef union {
    struct {
        uint64_t                Match  : 64; // Flit match bits for packet counter
    } field;
    uint64_t val;
} TXDMA_DBG_MC0_0_PKT_CNTR_FLIT_MATCH_t;
...
// TXDMA_DBG_MC0_1_PKT_CNTR_FLIT_MATCH desc:
typedef union {
    struct {
        uint64_t                Match  : 64; // Flit match bits for packet counter
    } field;
    uint64_t val;
} TXDMA_DBG_MC0_1_PKT_CNTR_FLIT_MATCH_t;
...
// TXDMA_DBG_MC0_2_PKT_CNTR_FLIT_MATCH desc:
typedef union {
    struct {
        uint64_t                Match  : 64; // Flit match bits for packet counter
    } field;
    uint64_t val;
} TXDMA_DBG_MC0_2_PKT_CNTR_FLIT_MATCH_t;
...
// TXDMA_DBG_MC0_3_PKT_CNTR_FLIT_MATCH desc:
typedef union {
    struct {
        uint64_t                Match  : 64; // Flit match bits for packet counter
    } field;
    uint64_t val;
} TXDMA_DBG_MC0_3_PKT_CNTR_FLIT_MATCH_t;
...
// TXDMA_DBG_MC1_0_PKT_CNTR_FLIT_MATCH desc:
typedef union {
    struct {
        uint64_t                Match  : 64; // Flit match bits for packet counter
    } field;
    uint64_t val;
} TXDMA_DBG_MC1_0_PKT_CNTR_FLIT_MATCH_t;
...
// TXDMA_DBG_MC1_1_PKT_CNTR_FLIT_MATCH desc:
typedef union {
    struct {
        uint64_t                Match  : 64; // Flit match bits for packet counter
    } field;
    uint64_t val;
} TXDMA_DBG_MC1_1_PKT_CNTR_FLIT_MATCH_t;
...
// TXDMA_DBG_MC0_PKT_CNTR_CTL desc:
typedef union {
    struct {
        uint64_t            tc_mask_0  :  2; // tc mask bits for packet counter 0, set bits are don't care
        uint64_t           tc_match_0  :  2; // tc match bits for packet counter 0
        uint64_t            tc_mask_1  :  2; // tc mask bits for packet counter 1, set bits are don't care
        uint64_t           tc_match_1  :  2; // tc match bits for packet counter 1
        uint64_t            tc_mask_2  :  2; // tc mask bits for packet counter 2, set bits are don't care
        uint64_t           tc_match_2  :  2; // tc match bits for packet counter 2
        uint64_t            tc_mask_3  :  2; // tc mask bits for packet counter 3, set bits are don't care
        uint64_t           tc_match_3  :  2; // tc match bits for packet counter 3
        uint64_t         flit_index_0  :  2; // flit index to match/mask on for packet counter 0, can only match on flits 0-3
        uint64_t         flit_index_1  :  2; // flit index to match/mask on for packet counter 1, can only match on flits 0-3
        uint64_t         flit_index_2  :  2; // flit index to match/mask on for packet counter 2, can only match on flits 0-3
        uint64_t         flit_index_3  :  2; // flit index to match/mask on for packet counter 3, can only match on flits 0-3
        uint64_t         pkt_cntr_ctl  :  3; // 0,5,6,7: counters are all independent 1: 0,1 are ganged (o enabling 1) and 2,3 are independent 2: 0,1 are ganged (0 enabling 1) and 2,3 are ganged (2 enabling 3) 3: 0,1,2 are ganged (0 enabling1, 1 enabling 2) and 3 is independent 4: 0,1,2,3 are ganged (0 enabling 1, 1 enabling 2, 2 enabling 3)
        uint64_t       Reserved_63_27  : 37; // Unused
    } field;
    uint64_t val;
} TXDMA_DBG_MC0_PKT_CNTR_CTL_t;

// TXDMA_DBG_MC1_PKT_CNTR_CTL desc:
typedef union {
    struct {
        uint64_t            tc_mask_0  :  2; // tc mask bits for packet counter 0, set bits are don't care
        uint64_t           tc_match_0  :  2; // tc match bits for packet counter 0
