...
typedef union {
    struct {
        ...
        uint64_t                  seo  :  1; // Force SO=1 on all events including spontaneous events for all MCTC's, including MCTC=0x8.
        uint64_t                   wo  :  8; // Force SO=1 for all Writes and all Atomics. Per MCTC. [0]=MC0TC0, [7]=MC1TC3.
        uint64_t                   ro  :  8; // Force SO=1 for all Buffered Reads and Fetching Atomics. Per MCTC. [0]=MC0TC0, [7]=MC1TC3.
        uint64_t                 rcwo  :  8; // Force SO=1 for all Ordered Writes and Ordered Non-fetching Atomics. Per MCTC. [0]=MC0TC0, [7]=MC1TC3.
        uint64_t                 rcro  :  8; // Force SO=1 for all Ordered Buffered Reads and all Ordered Atomics. Per MCTC. [0]=MC0TC0, [7]=MC1TC3.
        uint64_t                  lbs  :  8; // Force SO=1 on the Last Byte Write for multiple Cache Line Writes.
        uint64_t              gen1_do  :  8; // Force SO=1 for Gen-1 Put Events. Per MCTC. [0]=MC0TC0, [7]=MC1TC3.
    } field;
    uint64_t val;
} RXDMA_CFG_CONTROL_t;
...
typedef union {
    struct {
        ...
        uint64_t       Reserved_13_12  :  2; // Unused
...
typedef union {
    struct {
        ...
        uint64_t       hp_ack_sbe_cnt  : 16; // Count of SBE's for Ack commands from HP or OTR
        uint64_t       hp_ack_mbe_cnt  : 16; // Count of MBE's for Ack commands from HP or OTR
    } field;
    uint64_t val;
} RXDMA_ERR_INFO_4_t;
