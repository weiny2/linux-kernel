...
typedef union {
    struct {
        ...
        uint32_t         initiate_flr  :  1; // Initiate Function Level Reset. A write of 1 initiates Function Level Reset to the Function. HW clears this bit when FLR is complete.
        ...
        uint32_t    transactions_pend  :  1; // Transactions Pending. When Set, this bit indicates that the Function has pending transactions over IOSF-P or IDI.
...
typedef union {
    struct {
        ...
        uint32_t       poison_tlp_err  :  1; // Poisoned TLP Error. Posted Data Parity Error/Poison to P2SB NP Data Parity/Poison to CFGAlso reported in p2sb_p_data_ep p2sb_p_data_parity tarb_hdr_parity_error p2sb_np_data_ep p2sb_np_data_parity
        uint32_t               fc_err  :  1; // Flow Control Protocol Error.
        uint32_t     cmpl_timeout_err  :  1; // Completion Timeout Error. Pending ZBR request timeoutAlso reported in iosfp_timeout
        uint32_t       cmpl_abort_err  :  1; // Completion Abort Error. Illegal Length on MRd Illegal Length on CfgRdAlso reported in p2sb_np_read_err
        uint32_t        unexp_cpl_err  :  1; // Unexpected Completion Error. Unexpected completionAlso reported in p2sb_unexp_cmp
        uint32_t      rcv_overflw_err  :  1; // Receiver Overflow Error.
        uint32_t          mal_tlp_err  :  1; // Malformed TLP Error. Illegal Length on MWr Illegal Length on CfgWr Header Parity ErrorAlso reported in p2sb_data_len_err tarb_hdr_parity_error
        uint32_t             ecrc_err  :  1; // ERCR Error. Not supported
        uint32_t        unsup_req_err  :  1; // Unsupported Request Error Posted Unsupported opcode Posted illegal PCIe Byte Enables Non-posted Unsupported opcode Non-posted illegal PCIe Byte EnablesAlso reported in p2sb_p_unsup_req p2sb_np_unsup_req
        uint32_t    acs_violation_err  :  1; // ACS Violation Error. Not supported
        uint32_t       uncorr_int_err  :  1; // Uncorrectable Internal Error. FXR sourced viral
...
typedef union {
    struct {
        ...
        uint32_t advisory_nonfatal_err  :  1; // Advisory Non-Fatal Error Unexpected Completion Unsupported Request Non-Posted Unsupported Opcode Non-Posted Illegal PCIe BE Illegal Length on CfgRd Illegal Length on MRd Non-Posted Poison/Data Parity Error FXR ViralAlso reported in p2sb_unexp_cmp p2sb_np_unsup_req p2sb_np_read_err p2sb_np_data_ep p2sb_np_data_parity
