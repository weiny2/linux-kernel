...
typedef union {
    struct {
        uint64_t                  VAL  : 32; // Number of cclks at which lack of forward progress will take down link transfer active. This timer activates when the LCB enters the Rx Tossing State or is replaying link transfer packets. If the LCB does not make forward progress (does not exit from the Rx Tossing state) within the allowed timeout period, then link_transfer_active is cleared.
...
typedef union {
    struct {
        uint64_t                  VAL  :  1; // If set, the LCB uses cclk/2 instead of cclk to clock the dclk and rclk logic domains during reset, loopback and dfx modes.
        uint64_t          Unused_63_1  : 63; // Unused
    } field;
    uint64_t val;
...
typedef union {
    struct {
        uint64_t                 GEN2  :  1; // 
        uint64_t           Unused_3_1  :  3; // Unused
        uint64_t      USE_RETRYSEQNUM  :  1; // enables use of or tracking replay sequence numbers in idle flits
        uint64_t           Unused_7_5  :  3; // Unused
...
typedef union {
    struct {
        ...
        uint64_t                  SEQ  :  1; // Seq or Rolling mode
        uint64_t           Unused_3_2  :  2; // Unused
        uint64_t              ERR_LIM  : 24; // Allow less than # of errors within each sliding window. If equal or greater then increment a counter compared against the DEGRADE_LIM field.
        uint64_t          DEGRADE_LIM  :  4; // # of sequential ERR_LIM violations to trigger a lane degrade
        uint64_t              WIN_LIM  : 32; // Sliding window length in units of valid FEC decoder outputs
    } field;
    uint64_t val;
...
typedef union {
    struct {
        ...
        uint64_t           Unused_7_6  :  2; // Unused
        uint64_t    REQUIRED_KILL_CNT  :  5; // If tx_ton count does not meet a required turn on threshold and exceeds this count then the link is killed for lost turn on signal.
        uint64_t         Unused_15_13  :  3; // Unused
...
typedef union {
    struct {
        uint64_t       CRC_ERR_BIT_EN  : 32; // Allows bit to flip with enabled. TBD for more info
        uint64_t                 SEED  : 31; // 
...
typedef union {
    struct {
        ...
        uint64_t         Unused_43_41  :  3; // Unused
        uint64_t   REPLAY_BUF_ADDRESS  : 10; // Indicates the address within the replay buffer rams for which error injection is to occur. This field is used only when the error injection mode is 2 or 3. [36]: selects one of the two Qflits out that correspond to the indexed LTP in the RAM specified by RAMSELECT. There are 2 Qflits for every LTP in each of Replay_Buf_0 and Replay_Buf_1. [44:37]: selects the LTP [45]: Unused
...
typedef union {
    struct {
        ...
        uint64_t         Unused_63_59  :  5; // Unused
    } field;
    uint64_t val;
...
typedef union {
    struct {
        uint64_t RST_FOR_FAILED_DESKEW  :  8; // see ERR_FLG register Section 21.4.50, 'LCB_ERR_STS'
        uint64_t ALL_LNS_FAILED_REINIT_TEST  :  8; // see ERR_FLG register Section 21.4.50, 'LCB_ERR_STS'
        uint64_t RST_FOR_LINK_TIMEOUT  :  8; // see ERR_FLG register Section 21.4.50, 'LCB_ERR_STS'
        uint64_t RST_FOR_INCOMPLT_RND_TRIP  :  8; // see ERR_FLG register Section 21.4.50, 'LCB_ERR_STS'
        uint64_t         Unused_63_32  : 32; // Unused
    } field;
    uint64_t val;
...
typedef union {
    struct {
        ...
        uint64_t         Unused_19_18  :  2; // Unused
        uint64_t          CSR_CREATED  :  1; // Injected via CSR
        uint64_t         Unused_63_21  : 43; // Unused
    } field;
    uint64_t val;
...
typedef union {
    struct {
        uint64_t                  VAL  : 16; // This count is in units of LTP's. Informational: An LTP is transmitted on average every 5ns, for links running at 200Gbs.
        uint64_t         Unused_63_16  : 48; // Unused
    } field;
    uint64_t val;
...
typedef union {
    struct {
        uint64_t                  VAL  : 64; // When a MasterTimeLow flit, with an ID of 0, arrives at a port that supports a Local Clock the Local Clock value is captured in a LocalTime register. The LocalTime register is not changed when the MasterTimeHigh flit arrives. For a debug HFI FZC in port mirroring mode capture the Local Clock value in this register when a PortMirrorTime arrives with the LUT time stamp embedded.
    } field;
    uint64_t val;
...
typedef union {
    struct {
        uint64_t                  VAL  : 64; // When a MasterTimeLow flit, with an ID of 1, arrives at a port that supports a Local Clock the Local Clock value is captured in a LocalTime register. The LocalTime register is not changed when the MasterTimeHigh flit arrives. For a debug HFI FZC in port mirroring mode capture the Local Clock value in this register when a PortMirrorTime arrives with the LUT time stamp embedded.
    } field;
    uint64_t val;
...
typedef union {
    struct {
        uint64_t                  VAL  : 64; // When a MasterTimeLow flit, with an ID of 2, arrives at a port that supports a Local Clock the Local Clock value is captured in a LocalTime register. The LocalTime register is not changed when the MasterTimeHigh flit arrives. For a debug HFI FZC in port mirroring mode capture the Local Clock value in this register when a PortMirrorTime arrives with the LUT time stamp embedded.
    } field;
    uint64_t val;
...
typedef union {
    struct {
        uint64_t                  VAL  : 64; // When a MasterTimeLow flit, with an ID of 3, arrives at a port that supports a Local Clock the Local Clock value is captured in a LocalTime register. The LocalTime register is not changed when the MasterTimeHigh flit arrives. For a debug HFI FZC in port mirroring mode capture the Local Clock value in this register when a PortMirrorTime arrives with the LUT time stamp embedded.
    } field;
    uint64_t val;
