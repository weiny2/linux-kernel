...
typedef union {
    struct {
        uint64_t          itr_read_pe  :  1; // ITR read parity error. See itr_rpe_entry . ERR_CATEGORY_CORRECTABLE
        uint64_t        msix_addr_mbe  :  1; // MISX Table Addr MBE. See msix_addr_synd info. Request dropped. ERR_CATEGORY_NODE. SW recovery possible with polling of INT_STS
        ...
        uint64_t       p2sb_unexp_cmp  :  1; // P2SB Unexpected Completion. Response dropped.Dual reported in unexp_cpl_err or advisory_nonfatal_err ERR_CATEGORY_INFO
        uint64_t       p2sb_p_data_ep  :  1; // P2SB Posted Data poison. Request dropped. Dual reported in poison_tlp_err ERR_CATEGORY_HFI
        uint64_t   p2sb_p_data_parity  :  1; // P2SB Posted Data Parity error. Request dropped.Dual reported in poison_tlp_err ERR_CATEGORY_HFI
        uint64_t      p2sb_np_data_ep  :  1; // P2SB NonPosted Data poison error on CFGWr . Request dropped, and error response returned. Dual reported in poison_tlp_err or advisory_nonfatal_err ERR_CATEGORY_HFI
        uint64_t  p2sb_np_data_parity  :  1; // P2SB NonPosted Data Parity error on CFGWr . Request dropped, and error response returned. Dual reported in poison_tlp_err or advisory_nonfatal_err ERR_CATEGORY_HFI
        uint64_t      p2sb_p_addr_err  :  1; // P2SB Posted request Address Error. Address did not hit an FXR BAR. Request dropped. ERR_CATEGORY_HFI
        uint64_t    p2sb_data_len_err  :  1; // P2SB Posted Data Length Error. Request dropped.Root Causes: Illegal Length on MWr Illegal Length on CfgWrDual reported in mal_tlp_err ERR_CATEGORY_HFI. example would be P2P request to FXR.
        uint64_t     p2sb_np_read_err  :  1; // P2SB NonPosted read size error. CA response to IOSF-PRoot Causes: Illegal Length on CfgRd Illegal Length on MRdDual reported in cmpl_abort_err or advisory_nonfatal_err ERR_CATEGORY_HFI. example would be P2P request to FXR.
        uint64_t      p2sb_dataq0_mbe  :  1; // P2SB Data Queue0 MBE. Request dropped. ERR_CATEGORY_HFI
        uint64_t      p2sb_dataq0_sbe  :  1; // P2SB Data Queue0 SBE. ERR_CATEGORY_CORRECTABLE
        uint64_t      p2sb_dataq1_mbe  :  1; // P2SB Data Queue1 MBE. Request dropped. ERR_CATEGORY_HFI
        uint64_t      p2sb_dataq1_sbe  :  1; // P2SB Data Queue1 SBE. ERR_CATEGORY_CORRECTABLE
        uint64_t    sb2p_unsup_pc_fmt  :  1; // SB2P Unsupported Posted/Cmp Format. Transaction dropped. ERR_CATEGORY_HFI
        uint64_t    sb2p_unsup_pc_opc  :  1; // SB2P Unsupported Posted/Cmp Opcode. Transaction dropped. ERR_CATEGORY_HFI
        uint64_t    sb2p_unsup_np_fmt  :  1; // SB2P Unsupported NonPosted Format. UR response to IOSF-P ERR_CATEGORY_HFI
...
typedef union {
    struct {
        ...
        uint64_t            FZC_ERROR  :  1; // Frozen Creek Port0 IRQ
        uint64_t        OTR_PKT_ERROR  :  1; // OTR PKT Domain Error
        uint64_t          HIFIS_ERROR  :  1; // HIFIS Domain Error
        uint64_t           LOCA_ERROR  :  1; // HI Domain Error
        uint64_t           PCIM_ERROR  :  1; // PCIM Domain Error
        uint64_t          TXCID_ERROR  :  1; // TXCI Domain Error
        uint64_t          TXCIC_ERROR  :  1; // TXCI Domain Error
        uint64_t        OTR_MSG_ERROR  :  1; // OTR MSG Domain Error
        ...
        uint64_t          RXCID_ERROR  :  1; // RXCI Domain Error
        uint64_t          RXCIC_ERROR  :  1; // RXCI Domain Error
