...
typedef union {
    struct {
        ...
        uint64_t           rc_ack_tc0  :  3; // RC for acknowledgments on TC0
        uint64_t           rc_ack_tc1  :  3; // RC for acknowledgments on TC1
        uint64_t           rc_ack_tc2  :  3; // RC for acknowledgments on TC2
        uint64_t           rc_ack_tc3  :  3; // RC for acknowledgments on TC3
        uint64_t         rc_reply_tc0  :  3; // RC for replies on TC0
        uint64_t         rc_reply_tc1  :  3; // RC for replies on TC1
        uint64_t         rc_reply_tc2  :  3; // RC for replies on TC2
        uint64_t         rc_reply_tc3  :  3; // RC for replies on TC3
        uint64_t       Reserved_63_32  : 32; // Unused
    } field;
    uint64_t val;
} RXHP_CFG_RC_t;
