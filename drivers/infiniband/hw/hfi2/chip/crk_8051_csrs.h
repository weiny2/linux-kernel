...
typedef union {
    struct {
        ...
        uint64_t                 DRAM  :  1; // reset the 8051 data memory wrapper logic, when released, the data ram initialization logic is activated to init the ram, it takes maximum of 4096x4 cclk cycles to complete the process before it can be used by the 8051. During internal ram init time, the code ram can be loaded. If code ram loads faster than 4096x4 cycles required. the M8051W reset need to hold until the requirement met.
        uint64_t                 IRAM  :  1; // reset the 8051 internal data memory wrapper logic, when released, the internal ram initialization logic is activated to init the ram, it takes maximum of 260x4 cclk cycles to complete the process before it can be used by the 8051. During internal ram init time, the code ram can be loaded. If code ram loads faster than 256x4 cycles required. the M8051W reset need to hold until the requirement met.
...
typedef union {
    struct {
        uint64_t              GENERAL  :  8; // General configuration Register. bit 0: Reserved. bit 1: when set, self-GUID check is disabled. bit [7:2]: Reserved
        ...
        uint64_t           CLK_SELECT  :  2; // CRK_8051 clock divider configuration: 2'b00: reserved 2'b01: cclk/4 default 2'b10: reserved 2'b11: cclk/8
        uint64_t         Unused_63_26  : 38; // Unused
    } field;
    uint64_t val;
} CRK8051_CFG_MODE_t;
...
// CRK8051_CFG_SER_AN_LT_LANE_MUX desc:
typedef union {
    struct {
        uint64_t     lt_lane0_ack_sel  :  2; // mux select for lane0 ack
        uint64_t     lt_lane0_req_sel  :  2; // mux select for lane0 req
        uint64_t     lt_lane1_ack_sel  :  2; // mux select for lane1 ack
        uint64_t     lt_lane1_req_sel  :  2; // mux select for lane1 req
        uint64_t     lt_lane2_ack_sel  :  2; // mux select for lane2 ack
        uint64_t     lt_lane2_req_sel  :  2; // mux select for lane2 req
        uint64_t     lt_lane3_ack_sel  :  2; // mux select for lane3 ack
        uint64_t     lt_lane3_req_sel  :  2; // mux select for lane3 req
        uint64_t      an_lane0_tx_sel  :  2; // mux select for lane0 AN TX
        uint64_t      an_lane0_rx_sel  :  2; // mux select for lane0 AN RX
        uint64_t         Unused_23_20  :  4; // Unused
        uint64_t               enable  :  1; // enable the mux select
        uint64_t         Unused_63_25  : 39; // Unused
    } field;
    uint64_t val;
} CRK8051_CFG_SER_AN_LT_LANE_MUX_t;

// CRK8051_CFG_SPARE desc:
typedef union {
    struct {
...
typedef union {
    struct {
        uint64_t                  val  :  1; // When Set, indicates loss of the signal detected by any of 4 SerDes receivers.
        uint64_t           Unused_7_1  :  7; // Unused
        uint64_t       overide_enable  :  1; // When set, the SerDes LOS output is not used to set the val. It can be overridden by software.
        uint64_t          Unused_63_9  : 55; // Unused
    } field;
    uint64_t val;
...
// CRK8051_DBG_SER_INPUT desc:
typedef union {
    struct {
        uint64_t                  VAL  : 32; // SerDes Input 0
        uint64_t         Unused_63_32  : 32; // Unused
    } field;
    uint64_t val;
} CRK8051_DBG_SER_INPUT_t;

// CRK8051_DBG_DRAM_RD_ACCESS desc:
typedef union {
    struct {
        uint64_t              RD_DATA  : 64; // CRK_8051 Data RAM read data
    } field;
    uint64_t val;
} CRK8051_DBG_DRAM_RD_ACCESS_t;
