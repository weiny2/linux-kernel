				    ...
				    unsigned int group)
{
	...
	list_for_each_entry_safe((platform_msi_group), (tmp_platform_msi_group), dev_to_platform_msi_group_list((dev)), group_list) {
		if (((platform_msi_group)->group_id) == (group)) {
			list_for_each_entry_safe((desc), (tmp), &(platform_msi_group)->entry_list, list)        {
...

{
	...
	if (!list_empty(platform_msi_current_entry_list(dev))) {
		desc = list_last_entry(platform_msi_current_entry_list(dev),
	...
	for (i = 0; i < nvec; i++) {
		...
		list_add_tail(&desc->list, platform_msi_current_entry_list(dev));
	}
			     ...
			     const struct platform_msi_ops *platform_ops)
{
	...
	if (dev->is_platform_msi == GEN_PLAT_MSI) {
		if (!dev->msi_domain)
			return ERR_PTR(-EINVAL);
...
{
	dev->is_platform_msi = GEN_PLAT_MSI;
	return platform_msi_domain_alloc_irqs_group(dev, nvec, platform_ops, NULL);
}
					 ...
					 unsigned int *group_id)
{
	...
	list_add_tail(&platform_msi_group->group_list, dev_to_platform_msi_group_list(dev));
	...
	list_for_each_entry(platform_msi_group,
		...
		if (platform_msi_group->group_id == group) {
			if (!list_empty(&platform_msi_group->entry_list)) {
				...
				desc = list_first_entry(&(platform_msi_group)->entry_list, struct msi_desc, list);
			      ...
			      unsigned int nvec)
{
	...
	for_each_platform_msi_entry_in_group_safe(desc, tmp, platform_msi_group,
		tmp_platform_msi_group, data->dev->group_id, data->dev) {
