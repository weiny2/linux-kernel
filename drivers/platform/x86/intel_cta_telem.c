...
static void cta_telem_ep_release(struct kref *kref)
{
	struct telem_endpoint *ep;
...
EXPORT_SYMBOL_GPL(cta_telem_get_next_endpoint);
...
EXPORT_SYMBOL_GPL(cta_telem_register_endpoint);

void cta_telem_unregister_endpoint(struct telem_endpoint *ep)
{
	kref_put(&ep->kref, cta_telem_ep_release);
}
EXPORT_SYMBOL(cta_telem_unregister_endpoint);
...
EXPORT_SYMBOL_GPL(cta_telem_get_endpoint_info);

int
cta_telem_read32(struct telem_endpoint *ep, u32 offset, u32 *data, u32 count)
{
	void __iomem *base;
	u32 size;
...
EXPORT_SYMBOL_GPL(cta_telem_read32);

int
cta_telem_read64(struct telem_endpoint *ep, u32 offset, u64 *data, u32 count)
{
	void __iomem *base;
	u32 size;
...
static void cta_telem_populate_header(void __iomem *disc_offset,
				      struct telem_header *header)
{
...
static int cta_telem_probe(struct platform_device *pdev)
{
	...
	err = cta_telem_add_endpoint(priv);
	if (err)
		return err;
	...
	blocking_notifier_call_chain(&telem_notifier, CTA_TELEM_NOTIFY_ADD,
				     &priv->devid);
...
fail_alloc_chrdev:
	kref_put(&priv->ep->kref, cta_telem_ep_release);
...
static int cta_telem_remove(struct platform_device *pdev)
{
	...
	blocking_notifier_call_chain(&telem_notifier, CTA_TELEM_NOTIFY_REMOVE,
				     &priv->devid);
