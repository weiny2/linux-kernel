...
/*
 * Intel Converged Telemetry Aggregator Watcher driver
 *
...
static inline bool cta_watcher_is_sampler(struct watcher_endpoint *ep)
{
	return ep->header.watcher_type == TYPE_SAMPLER1 ||
	       ep->header.watcher_type == TYPE_SAMPLER2;
}

static inline bool cta_watcher_is_tracer(struct watcher_endpoint *ep)
{
	return ep->header.watcher_type == TYPE_TRACER1 ||
	       ep->header.watcher_type == TYPE_TRACER2;
}

static inline bool cta_watcher_select_limited(struct watcher_endpoint *ep)
{
	return cta_watcher_is_sampler(ep) ||
	       ep->header.watcher_type == TYPE_TRACER2;
}

static inline bool cta_watcher_is_type2(struct watcher_endpoint *ep)
{
 ...
 * I/O
 */
static bool cta_watcher_request_pending(struct watcher_endpoint *ep)
{
	/*
...
static bool cta_watcher_in_use(struct watcher_endpoint *ep)
{
	/*
	 * Read request pending bit into temporary location so we can read the
	 * pending bit without overwriting other settings. If a collection is
...
void
cta_watcher_write_vector_to_dev_type2(struct watcher_endpoint *ep)
{
...
{
	if (cta_watcher_is_type2(ep))
		cta_watcher_write_vector_to_dev_type2(ep);
	else
		cta_watcher_write_vector_to_dev_type1(ep);
}
/*
 * devfs
 */
static int cta_watcher_sampler_open(struct inode *inode, struct file *filp)
{
	struct watcher_endpoint *ep;
	struct pci_driver *pci_drv;
	struct cta_watcher_priv *priv;

	if (!capable(CAP_SYS_ADMIN))
		return -EPERM;

	ep = container_of(inode->i_cdev, struct watcher_endpoint, cdev);
	priv = container_of(ep, struct cta_watcher_priv, ep);
...
static int cta_watcher_sampler_release(struct inode *inode, struct file *filp)
{
...
static int
cta_watcher_sampler_mmap(struct file *filp, struct vm_area_struct *vma)
{
	struct watcher_endpoint *ep = filp->private_data;
	struct cta_watcher_priv *priv;
...
static const struct file_operations cta_watcher_sampler_fops = {
	.owner =	THIS_MODULE,
	.open =		cta_watcher_sampler_open,
	.mmap =		cta_watcher_sampler_mmap,
	.release =	cta_watcher_sampler_release,
};
	   ...
	   const char *buf, size_t count)
{
	...
	if (cta_watcher_request_pending(ep))
		return -EBUSY;
	...
	cta_watcher_write_ctrl_to_dev(ep);
			 ...
			 unsigned long *bit_vector)
{
	 ...
	 */
	if (cta_watcher_select_limited(ep)) {
...
static struct class cta_watcher_class = {
	.name = "cta_watcher",
	.owner = THIS_MODULE,
	.dev_groups = cta_watcher_groups,
};

/*
 * initialization
 */
static int cta_watcher_make_dev(struct cta_watcher_priv *priv)
{
	...
	if (cta_watcher_is_sampler(ep)) {
		cdev_init(&ep->cdev, &cta_watcher_sampler_fops);
	...
	dev = device_create(&cta_watcher_class, priv->dev, ep->devt, ep,
			    "%s%d", name, ep->devid);
...
static int
cta_watcher_create_endpoint(struct cta_watcher_priv *priv)
{
	struct watcher_endpoint *ep = &priv->ep;
...
static void
cta_watcher_populate_header(void __iomem *disc_offset,
			    struct watcher_header *header)
{
...
static int cta_watcher_probe(struct platform_device *pdev)
{
	struct cta_watcher_priv *priv;
	struct watcher_endpoint *ep;
	int err;
	...
	cta_watcher_populate_header(priv->disc_table, &priv->ep.header);
...
static int cta_watcher_remove(struct platform_device *pdev)
{
	struct cta_watcher_priv *priv;
	struct watcher_endpoint *ep;

	priv = (struct cta_watcher_priv *)platform_get_drvdata(pdev);
	ep = &priv->ep;

	device_destroy(&cta_watcher_class, ep->devt);
	if (cta_watcher_is_sampler(ep))
		cdev_del(&ep->cdev);
...
static struct platform_driver cta_watcher_driver = {
	.driver = {
		.name   = DRV_NAME,
	},
	.probe  = cta_watcher_probe,
	.remove = cta_watcher_remove,
};

static int __init cta_watcher_init(void)
{
	int ret = class_register(&cta_watcher_class);

	if (ret)
		return ret;

	ret = platform_driver_register(&cta_watcher_driver);
	if (ret) {
		class_unregister(&cta_watcher_class);
		return ret;
	}
...
static void __exit cta_watcher_exit(void)
{
	platform_driver_unregister(&cta_watcher_driver);
	class_unregister(&cta_watcher_class);
	ida_destroy(&sampler_devid_ida);
	ida_destroy(&tracer_devid_ida);
}

module_init(cta_watcher_init);
module_exit(cta_watcher_exit);
