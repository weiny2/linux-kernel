...
/*
 * Intel Converged Telemetry Crashlog driver
 *
 ...
 * I/O
 */
static bool cta_crashlog_complete(struct crashlog_endpoint *ep)
{
	struct cta_crashlog_priv *priv = container_of(ep,
						      struct cta_crashlog_priv,
						      ep);
...
static bool cta_crashlog_disabled(struct crashlog_endpoint *ep)
{
	struct cta_crashlog_priv *priv = container_of(ep,
						      struct cta_crashlog_priv,
						      ep);
...
static void cta_crashlog_set_disable(struct crashlog_endpoint *ep, bool disable)
{
	struct cta_crashlog_priv *priv = container_of(ep,
						      struct cta_crashlog_priv,
						      ep);
...
static void cta_crashlog_set_clear(struct crashlog_endpoint *ep)
{
	struct cta_crashlog_priv *priv = container_of(ep,
						      struct cta_crashlog_priv,
						      ep);
...
static void cta_crashlog_set_execute(struct crashlog_endpoint *ep)
{
	struct cta_crashlog_priv *priv = container_of(ep,
						      struct cta_crashlog_priv,
						      ep);
 ...
 * devfs
 */
static int cta_crashlog_open(struct inode *inode, struct file *filp)
{
	struct crashlog_endpoint *ep;
	struct pci_driver *pci_drv;
	struct cta_crashlog_priv *priv;

	if (!capable(CAP_SYS_ADMIN))
		return -EPERM;

	ep = container_of(inode->i_cdev, struct crashlog_endpoint, cdev);
	priv = container_of(ep, struct cta_crashlog_priv, ep);
...
static int cta_crashlog_release(struct inode *inode, struct file *filp)
{
...
static int
cta_crashlog_mmap(struct file *filp, struct vm_area_struct *vma)
{
	struct crashlog_endpoint *ep = filp->private_data;
	struct cta_crashlog_priv *priv;
...
static const struct file_operations cta_crashlog_fops = {
	.owner =	THIS_MODULE,
	.open =		cta_crashlog_open,
	.mmap =		cta_crashlog_mmap,
	.release =	cta_crashlog_release,
};
...
enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ep = dev_get_drvdata(dev);
	enabled = !cta_crashlog_disabled(ep);
	    ...
	    const char *buf, size_t count)
{
	...
	cta_crashlog_set_disable(ep, !enabled);
...
trigger_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ep = dev_get_drvdata(dev);
	trigger = cta_crashlog_complete(ep);
	    ...
	    const char *buf, size_t count)
{
	...
	if (trigger) {
		...
		if (cta_crashlog_complete(ep))
			return -EEXIST;

		/* if device is currently disabled, return busy */
		if (cta_crashlog_disabled(ep))
			return -EBUSY;

		cta_crashlog_set_execute(ep);
	} else {
		cta_crashlog_set_clear(ep);
	}
...
static struct attribute *cta_crashlog_attrs[] = {
	&dev_attr_guid.attr,
	&dev_attr_size.attr,
...
ATTRIBUTE_GROUPS(cta_crashlog);

static struct class cta_crashlog_class = {
	.name = "cta_crashlog",
	.owner = THIS_MODULE,
	.dev_groups = cta_crashlog_groups,
};

/*
 * initialization
 */
static int cta_crashlog_make_dev(struct cta_crashlog_priv *priv)
{
	...
	cdev_init(&ep->cdev, &cta_crashlog_fops);
	...
	dev = device_create(&cta_crashlog_class, priv->dev, ep->devt, ep,
...
static void
cta_crashlog_populate_header(void __iomem *disc_offset,
			     struct crashlog_header *header)
{
...
static int cta_crashlog_probe(struct platform_device *pdev)
{
	struct cta_crashlog_priv *priv;
	struct crashlog_endpoint *ep;
	int err;
	...
	cta_crashlog_populate_header(priv->disc_table, &priv->ep.header);
	...
	err = cta_crashlog_make_dev(priv);
	if (err) {
...
static int cta_crashlog_remove(struct platform_device *pdev)
{
	struct cta_crashlog_priv *priv;
	struct crashlog_endpoint *ep;

	priv = (struct cta_crashlog_priv *)platform_get_drvdata(pdev);
	ep = &priv->ep;

	device_destroy(&cta_crashlog_class, ep->devt);
	cdev_del(&ep->cdev);
...
static struct platform_driver cta_crashlog_driver = {
	.driver = {
		.name   = DRV_NAME,
	},
	.probe  = cta_crashlog_probe,
	.remove = cta_crashlog_remove,
};

static int __init cta_crashlog_init(void)
{
	int ret = class_register(&cta_crashlog_class);

	if (ret)
		return ret;

	ret = platform_driver_register(&cta_crashlog_driver);
	if (ret) {
		class_unregister(&cta_crashlog_class);
		return ret;
	}
...
static void __exit cta_crashlog_exit(void)
{
	platform_driver_unregister(&cta_crashlog_driver);
	class_unregister(&cta_crashlog_class);
	ida_destroy(&crashlog_devid_ida);
}

module_init(cta_crashlog_init);
module_exit(cta_crashlog_exit);

MODULE_AUTHOR("Alexander Duyck <alexander.h.duyck@linux.intel.com>");
MODULE_DESCRIPTION("Intel CTA Crashlog driver");
