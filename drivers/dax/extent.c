...
int __must_check dr_extent_get_not_zero(struct dr_extent *dr_extent)
{
	return kref_get_unless_zero(&dr_extent->ref);
}
...
static void dr_ext_reg_put(void *data)
{
			      ...
			      const char *label)
{
	...
	dev->id = offset / dax_region->align;
