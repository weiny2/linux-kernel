			     ...
			     struct dr_extent *dr_extent)
{
	...
	dev_dbg(dev, "Trying notify: type %d HPA %#llx - %#llx\n",
		event, dr_extent->hpa_range.start, dr_extent->hpa_range.end);
	...
	if (dev->driver) {
		...
		if (reg_drv->notify) {
			dev_dbg(dev, "Notify: type %d HPA %#llx - %#llx\n",
				event, dr_extent->hpa_range.start,
				dr_extent->hpa_range.end);
...
static int cxl_rm_dr_ext_by_range(struct device *dev, void *data)
{
	 ...
	 */
	if (range_overlaps(rm_data->range, &dr_extent->hpa_range)) {
		...
		cxl_ed_rm_dr_extent(cxlr, dr_extent);
	}
	return 0;
}
