...
void memdev_release_extent(struct cxl_memdev_state *mds, struct range *range)
{
	...
	dev_dbg(dev, "Releasing extent %par\n", range);

	xa_init(&extent_list);
	if (xa_insert(&extent_list, 0, &extent, GFP_KERNEL)) {
		dev_dbg(dev, "Failed to release %par; %d\n", range, rc);
		goto error;
	}

	rc = cxl_send_dc_response(mds, CXL_MBOX_OP_RELEASE_DC, &extent_list, 1);
	if (rc)
		dev_dbg(dev, "Failed to release %par; %d\n", range, rc);
			       ...
			       struct cxl_extent *extent)
{
	...
	rc = cxl_validate_extent(mds, cxled, extent);
	if (rc)
		return rc;

	return cxled_add_extent(cxled, extent);
}
			   ...
			   struct cxl_endpoint_decoder *cxled)
{
	...
	xa_for_each(&mds->pending_extents, index, extent) {
		if (validate_add_extent(mds, cxled, extent)) {
			    ...
			    struct cxl_event_dcd *event)
{
	...
	return cxl_add_pending(mds, cxled);
}

static int handle_release_event(struct cxl_endpoint_decoder *cxled,
				struct cxl_event_dcd *event)
{
	return cxled_release_extent(cxled, &event->extent);
}
					...
					struct cxl_event_record_raw *raw_rec)
{
	...
	case DCD_ADD_CAPACITY:
		return handle_add_event(mds, cxled, event);
	case DCD_RELEASE_CAPACITY:
		return handle_release_event(cxled, event);
	case DCD_FORCED_CAPACITY_RELEASE:
...
static int __cxled_add_extents(struct cxl_endpoint_decoder *cxled)
{
	...
	do {
		...
		get_extent = (struct cxl_mbox_get_extent_in) {
			.extent_cnt = max(max_extent_count,
					cpu_to_le32(total_expected - current_index)),
		...
		for (int i = 0; i < nr_returned ; i++) {
			...
			rc = validate_add_extent(mds, cxled, extent);
			if (rc)
				continue;
		}
...
/**
 * cxled_add_extents() - Read existing extents
