...
static void ieh_output_error(struct decoded_res *res)
{
	...
	if (res->sev != IEH_FA_ERR)
		return;

	switch (ieh_cfg->op_on_fa) {
	case RESTART:
		WARN(1, "Restart system on device fatal error!\n");
		kernel_restart(NULL);
		break;

	case POWER_OFF:
		WARN(1, "Power off system on device fatal error!\n");
		kernel_power_off();
		break;
	default:
		break;
