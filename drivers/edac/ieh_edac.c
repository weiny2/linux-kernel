 ...
 * Tiger Lake-LP. This driver reads/prints the error severity and error source
...
({									\
	int rc = pci_read_config_dword(pdev, addr, val);		\
	if (rc)								\
...
({									\
	int rc = pci_write_config_dword(pdev, addr, val);		\
	if (rc)								\
...
{
	if (PCI_RD_U32(pdev, offset, val))
		return -ENODEV;

	/* Write 1s to clear status */
	if (PCI_WR_U32(pdev, offset, *val))
		return -ENODEV;
...
#define UNMASK_ERR_EVENT(ieh, name)					\
	do {								\
		u32 val;						\
		if (PCI_RD_U32(ieh->pdev, name##MSK_OFFSET, &val))	\
			return -ENODEV;					\
		val &= ~name##MSK;					\
		PCI_WR_U32(ieh->pdev, name##MSK_OFFSET, val);		\
	} while (0)
...
static int ieh_handle_error(struct ieh_dev *d, enum severity_level sev)
{
	...
	while ((i = dev_idx(sts, start)) != -1) {
		...
		PCI_RD_U32(d->pdev, DEVFUN_OFFSET + i * 4, &reg);
...
static void __ieh_check_error(struct ieh_dev *ieh)
{
	...
	if (PCI_RD_U32(pdev, GSYSEVTSTS_OFFSET, &sts))
		return;
...
static int __get_all_iehs(u16 did)
{
	...
	for (;;) {
		...
		case IEH_SUPERSET:
		case IEH_GLOBAL:
			PCI_RD_U32(pdev, GSYSEVTMAP_OFFSET, &reg);
