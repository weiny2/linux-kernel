...
{
	struct kvm_gmem_inode *i_gmem = file->f_inode->i_private;
...
{
	struct kvm_gmem_inode *i_gmem = inode->i_private;
				  ...
				  gfn_t gfn, struct folio *folio, int max_order)
{
	 ...
	 * the folio backs, it's possible that later on the same folio might be
	 * accessed with a larger order when it becomes possible to map the
	 * full GPA range into the guest using a larger order.  In such cases,
	 * some sub-ranges might already have been prepared.
	 *
...
/* FIXME IKW this pgoff may be wrong. */
		clear_highpage(folio_page(folio, vmf->pgoff));
/* IKW fault a single page at a time for now */
		kvm_gmem_mark_prepared(file, vmf->pgoff, 0);
	}
						      ...
						      loff_t size, u64 flags)
{
	...
	struct kvm_gmem_inode *ginode __free(kfree) =
			kvzalloc(KVM_GMEM_INODE_SIZE(size), GFP_KERNEL);
	if (!ginode)
