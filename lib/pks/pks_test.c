...
static bool run_on_boot; /* Default '= false' which checkpatch complains about
			    initalizing; so we don't */
			      ...
			      size_t count, loff_t *ppos)
{
	...
	if (!strcmp(buf, "2") && ctx->pks_cpu_enabled) {
		unsigned long reg_pks;
		int access;

		rdmsrl(MSR_IA32_PKS, reg_pks);

		access = (reg_pks >> (ctx->pkey * PKRU_BITS_PER_PKEY)) &
			  PKEY_ACCESS_MASK;
