...
typedef union {
    struct {
        ...
        uint64_t START_AUTHENTICATION  :  1; // When written to 1 starts the authentication process.
        uint64_t AUTHENTICATION_COMPLETE  :  1; // The authentication process has completed. Cleared by hardware when START_AUTHENTICATION is set or the hdr_pe code RAM is written .
        uint64_t AUTHENTICATION_SUCCESS  :  1; // Authentication was successful, written by hardware when authentication is completed, cleared by hardware when code RAM is written or START_AUTHENTICATION is
        uint64_t       Reserved_63_19  : 45; // Unused
    } field;
    uint64_t val;
} RXHP_CFG_HDR_PE_t;
...
// RXHP_DBG_PD_RAM desc:
typedef union {
    struct {
        uint64_t                 data  : 64; // For instruction memory writes, bits 63:40 are ignored, 39:32 are the ECC to write, and 31:0 are the instruction data. For data memory writes, all 64 bits are used and ECC is generated by hardware.
    } field;
    uint64_t val;
} RXHP_DBG_PD_RAM_t;
